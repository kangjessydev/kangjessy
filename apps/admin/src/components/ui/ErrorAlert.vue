    ...
    <div v-if="error" class="error-alert" :class="alertClass">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          <h4 class="font-black text-sm mb-1 uppercase tracking-wide" v-if="error.code">
            {{ error.code.replace(/_/g, ' ') }}
          </h4>
          <p class="text-sm font-medium">{{ error.message }}</p>
          <p v-if="showDetails && error.details" class="text-xs opacity-60 mt-1 font-mono">
...
import { AlertCircle, XCircle, AlertTriangle, Info, RotateCw, X } from 'lucide-vue-next'
