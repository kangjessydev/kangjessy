    ...
    <section id="services" class="bg-bg-primary" :class="[paddingClass]">
        <div class="container mx-auto px-4" :class="containerClass">
            ...
            <div class="hidden md:grid grid-cols-12 gap-6">
                <ServiceCard v-for="(service, index) in sortedServices" :key="service.id" :service="service"
            ...
            <div class="md:hidden">
                ...
                <div ref="sliderRef" @scroll="handleScroll"
                    ...
                    <ServiceCard v-for="service in sortedServices" :key="'mob-' + service.id" :service="service"
                        custom-class="snap-center shrink-0 w-[calc(100%-20px)] shadow-lg" />
                </div>

                <div class="flex justify-center gap-2 mt-2.5">
                    <span v-for="(_, i) in sortedServices" :key="i" class="h-2 rounded-full transition-all duration-300"
...
const props = defineProps({
    ...
    subtitle: {
        type: String,
        default: 'Gak ada solusi yang "one size fits all". Saya bangun sistem yang emang dirancang khusus buat nyelesein masalah unik di bisnis kamu.'
    },
    showHeader: {
        type: Boolean,
        default: true
...
const formattedTitle = computed(() => {
    return props.title.replace('Spesifik', '<span class="bg-linear-to-r from-accent-primary to-accent-secondary bg-clip-text text-transparent">Spesifik</span>');
});

const sortedServices = computed(() => {
    return Object.values(servicesData).sort((a, b) => (a.order || 99) - (b.order || 99));
});
