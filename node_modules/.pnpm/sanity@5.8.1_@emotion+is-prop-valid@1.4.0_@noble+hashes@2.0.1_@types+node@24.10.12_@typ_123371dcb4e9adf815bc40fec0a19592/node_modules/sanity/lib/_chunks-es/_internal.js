...
import fs, { createWriteStream, mkdirSync, existsSync, createReadStream } from "node:fs";
import fs$1, { mkdtemp, writeFile, readdir } from "node:fs/promises";
import os, { tmpdir } from "node:os";
import path from "node:path";
import { pipeline, finished } from "node:stream/promises";
import { absolutify } from "@sanity/util/fs";
import { Mutex } from "async-mutex";
import { isString, padStart, noop as noop$1, isEqual, isPlainObject, groupBy, deburr, sortBy, size } from "lodash-es";
...
import { isAfter, lightFormat, parse, isValid, formatDistanceToNow, parseISO, formatDistance } from "date-fns";
...
const debug$2 = debug$3("sanity:core");
...
var debug$1 = debug$3("sanity:backup");
...
const CONNECTION_TIMEOUT$1 = 15 * 1e3, READ_TIMEOUT$1 = 180 * 1e3, request$1 = getIt([keepAlive(), promise()]);
async function downloadAsset(url2, fileName, fileType, outDir) {
  const normalizedFileName = path.basename(fileName), assetFilePath = getAssetFilePath(normalizedFileName, fileType, outDir);
  await withRetry(async () => {
    ...
    debug$1("Received asset %s with status code %d", normalizedFileName, response?.statusCode), await pipeline(response.body, createWriteStream(assetFilePath));
  });
}
function getAssetFilePath(fileName, fileType, outDir) {
  let assetFilePath = "";
  return fileType === "image" ? assetFilePath = path.join(outDir, "images", fileName) : fileType === "file" && (assetFilePath = path.join(outDir, "files", fileName)), assetFilePath;
}
const CONNECTION_TIMEOUT = 15 * 1e3, READ_TIMEOUT = 180 * 1e3, request = getIt([keepAlive(), promise()]);
...
const debug = debug$3("sanity:backup"), DEFAULT_DOWNLOAD_CONCURRENCY = 10, MAX_DOWNLOAD_CONCURRENCY = 24, helpText$J = `
Options
...
function parseCliFlags$7(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("backup-id", {
    type: "string"
  }).options("out", {
    type: "string"
  }).options("concurrency", {
...
const downloadBackupCommand = {
  ...
  action: async (args, context) => {
    ...
    const start = Date.now(), progressSpinner = newProgress(output, "Setting up backup environment..."), tmpOutDir = await mkdtemp(path.join(tmpdir(), "sanity-backup-"));
    ...
    const tmpOutDocumentsFile = path.join(tmpOutDir, "data.ndjson"), docOutStream = createWriteStream(tmpOutDocumentsFile), docWriteMutex = new Mutex();
    try {
...
`);
    ...
    docOutStream.end(), await finished(docOutStream), progressSpinner.set({
...
async function prepareBackupOptions(context, args) {
  ...
  let out = await (async () => flags.out !== void 0 ? absolutify(flags.out) : await prompt.single({
    type: "input",
    message: "Output path:",
    default: path.join(workDir, defaultOutFileName),
    filter: absolutify
  }))();
  return isPathDirName(out) && (out = path.join(out, defaultOutFileName)), !flags.overwrite && existsSync(out) && (await prompt.single({
    type: "confirm",
...
function parseCliFlags$6(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("after", {
    type: "string"
  }).options("before", {
    type: "string"
  }).options("limit", {
...
async function getBuildAction(context) {
  return determineIsApp(context.cliConfig) ? (await import("./buildAction.js")).default : (await import("./buildAction2.js")).default;
}
...
function promptForCredentials(hasWildcard, context) {
  ...
  return output.print(""), hasWildcard ? output.print(oneline`
...
function filterOrigin(origin) {
  if (origin === "*" || origin === "file:///*" || origin === "null")
    return origin;
  try {
    const example = origin.replace(/([^:])\*/g, `$1${wildcardReplacement}`).replace(/:\*/, portReplacement), parsed = url.parse(example);
...
`, addCorsOriginCommand = {
  ...
  action: async (args, context) => {
    ...
    fs.existsSync(path.join(process.cwd(), origin)) && output.warn(`Origin "${origin}?" Remember to quote values (sanity cors add "*")`), await addCorsOrigin(origin, flags, context) && output.print("CORS origin added successfully");
  }
}, corsGroup = {
  name: "cors",
  signature: "[COMMAND]",
...
`);
  ...
  if (response && response.length > 0) {
    ...
    response.forEach((job) => {
      ...
      let timeStarted = "";
      createdAt !== "" && (timeStarted = formatDistanceToNow(parseISO(createdAt)));
      let timeTaken = "";
      updatedAt !== "" && (timeTaken = formatDistance(parseISO(updatedAt), parseISO(createdAt)));
      let color;
      switch (state) {
        case "completed":
          color = "green";
...
const progress = (url2) => new Observable((observer) => {
  let progressSource = new EventSource(url2), stopped = !1;
  function onError(error) {
    progressSource && progressSource.close(), debug$2(`Error received: ${error}`), !stopped && (observer.next({
      type: "reconnect"
    }), progressSource = new EventSource(url2));
  }
...
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  let dstStats = await fs$1.stat(dstPath).catch(noop);
  ...
  if (!dstStats) {
    ...
    await fs$1.mkdir(createPath, {
      recursive: !0
    });
  }
  const finalPath = looksLikeFile ? dstPath : path.join(dstPath, `${dataset}.tar.gz`);
  return dstStats = await fs$1.stat(finalPath).catch(noop), !flags.overwrite && dstStats && dstStats.isFile() && !await prompt.single({
    type: "confirm",
...
const MINIMUM_API_VERSION = "2025-02-19", importDatasetCommand = {
  ...
  action: async (args, context) => {
    const {
      ...
      allowSystemDocuments,
      replaceAssets
    } = flags, operation = getMutationOperation(args.extOptions), releasesOperation = getReleasesOperation(flags), client = apiClient(), [file, target] = args.argsWithoutOptions;
    if (!file)
    ...
    if (isUrl)
      ...
      const sourceFile = path.resolve(process.cwd(), file), fileStats = await fs$1.stat(sourceFile).catch(() => null);
      if (!fileStats)
        throw new Error(`${sourceFile} does not exist or is not readable`);
      sourceIsFolder = fileStats.isDirectory(), sourceIsFolder ? inputStream = sourceFile : (assetsBase = path.dirname(sourceFile), inputStream = createReadStream(sourceFile));
    }
...
function getReleasesOperation(flags) {
  const {
    replace,
    missing
  } = flags;
  return replace ? "replace" : missing ? "ignore" : "fail";
}
...
Examples
  ...
  sanity deploy --external`, deployCommand = {
  name: "deploy",
  ...
  action: async (args, context) => {
    let mod;
    return determineIsApp(context.cliConfig) ? mod = await import("./deployAction.js") : mod = await import("./deployAction2.js"), mod.default(args, context);
  },
  helpText: helpText$w
}, helpText$v = `
Options
...
`, undeployCommand = {
  ...
  action: async (args, context) => {
    let mod;
    return determineIsApp(context.cliConfig) ? mod = await import("./undeployAction.js") : mod = await import("./undeployAction2.js"), mod.default(args, context);
  },
  helpText: helpText$v
};
createRequire(import.meta.url);
...
async function getDevAction(context) {
  return determineIsApp(context.cliConfig) ? (await import("./devAction.js")).default : (await import("./devAction2.js").then(function(n) {
    return n.devAction;
  })).default;
}
const helpText$t = `
...
`, createDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    if ((replace || missing) && (operation = replace ? "createOrReplace" : "createIfNotExists"), file) {
      const contentPath = path.resolve(process.cwd(), file), content = json5.parse(await fs$1.readFile(contentPath, "utf8")), result = await writeDocuments(content, operation, client);
      output.print(getResultMessage(result, operation));
      return;
    }
    const docId = id || uuid(), ext = useJson5 ? "json5" : "json", tmpFile = path.join(os.tmpdir(), "sanity-cli", `${docId}.${ext}`), stringify = useJson5 ? json5.stringify : JSON.stringify, defaultValue = id && await client.getDocument(id) || {
      _id: docId,
      _type: "specify-me"
    };
    await fs$1.mkdir(path.join(os.tmpdir(), "sanity-cli"), {
      recursive: !0
    }), await fs$1.writeFile(tmpFile, stringify(defaultValue, null, 2), "utf8");
    const editor = getEditor();
    watch ? (registerUnlinkOnSigInt(tmpFile), output.print(`Watch mode: ${tmpFile}`), output.print("Watch mode: Will write documents on each save."), output.print("Watch mode: Press Ctrl + C to cancel watch mode."), chokidar.watch(tmpFile).on("change", () => (output.print(""), readAndPerformCreatesFromFile(tmpFile))), await execa(editor.bin, editor.args.concat(tmpFile), {
      stdio: "inherit"
    })) : (execa.sync(editor.bin, editor.args.concat(tmpFile), {
      stdio: "inherit"
    }), await readAndPerformCreatesFromFile(tmpFile), await fs$1.unlink(tmpFile).catch(noop$1));
    async function readAndPerformCreatesFromFile(filePath) {
      let content;
      try {
        content = json5.parse(await fs$1.readFile(filePath, "utf8"));
      } catch (err) {
...
function registerUnlinkOnSigInt(tmpFile) {
  process.on("SIGINT", async () => {
    await fs$1.unlink(tmpFile).catch(noop$1), process.exit(130);
  });
}
...
`, deleteDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    try {
      ...
      deleted.length > 0 && output.print(`Deleted ${deleted.length} ${pluralize("document", deleted.length)}`), notFound.length > 0 && output.error(chalk.red(`${pluralize("Document", notFound.length)} not found: ${notFound.join(", ")}`));
    } catch (err) {
...
`, validateDocumentsCommand$1 = {
  ...
  helpText: helpText$p,
  action: async (args, context) => (await import("./validateAction2.js")).default(args, context)
}, helpText$o = `
Options
...
`, execCommand = {
  ...
  helpText: helpText$o,
  action: async (args, context) => (await import("./execScript.js")).default(args, context)
}, helpText$n = `
Options
...
`, deleteGraphQLAPICommand = {
  ...
  action: async (args, context) => (await import("./deleteApiAction.js")).default(args, context),
  helpText: helpText$n
}, helpText$m = `
Options
...
`, deployGraphQLAPICommand = {
  ...
  action: async (args, context) => (await import("./deployApiAction.js")).default(args, context),
  helpText: helpText$m
}, graphqlGroup = {
  name: "graphql",
  signature: "[COMMAND]",
...
`, listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => (await import("./listApisAction.js")).default(args, context),
  helpText: helpText$l
}, createHookCommand = {
  name: "create",
  group: "hook",
  ...
  action: async (args, context) => {
    ...
    output.print(`Opening ${manageUrl}`), await open(manageUrl);
  }
}, deleteHookCommand = {
  name: "delete",
  group: "hook",
...
${err.message}`, {
    ...
    const groupedAttempts = groupBy(attempts, "messageId"), populated = messages.map((msg) => ({
      ...msg,
...
function printMessage(message, context, options) {
  ...
  output.print(`Date: ${message.createdAt}`), output.print(`Status: ${message.status}`), output.print(`Result code: ${message.resultCode}`), message.failureCount > 0 && output.print(`Failures: ${message.failureCount}`), detailed && (output.print("Payload:"), output.print(inspect(JSON.parse(message.payload), {
    colors: !0
...
`, extractManifestCommand = {
  ...
  action: async (args, context) => {
    const {
      extractManifestSafe
    } = await import("./extractManifestAction.js"), extractError = await extractManifestSafe(args, context);
    if (extractError)
      throw extractError;
    return extractError;
  }
...
`, createAspectCommand = {
  ...
  helpText: helpText$j,
  action: async (args, context) => (await import("./createAspectAction.js")).default(args, context)
}, helpText$i = `
Options
...
`, deleteAspectCommand = {
  ...
  helpText: helpText$i,
  action: async (args, context) => (await import("./deleteAspectAction.js")).default(args, context)
}, helpText$h = `
Options
  --media-library-id The id of the target media library.
...
`, deployAspectCommand = {
  ...
  helpText: helpText$h,
  action: async (args, context) => (await import("./deployAspectAction.js")).default(args, context)
}, helpText$g = `
Options
  --media-library-id The id of the target media library.
...
`, exportMediaCommand = {
  ...
  group: "media",
  signature: "[FILE]",
  description: "Export an archive of all file and image assets including their aspect data from the target media library.",
  helpText: helpText$g,
  action: async (args, context) => (await import("./exportAssetsAction.js")).default(args, context)
}, helpText$f = `
Options
  --media-library-id The id of the target media library.
...
`, importMediaCommand = {
  ...
  helpText: helpText$f,
  action: async (args, context) => (await import("./importAssetsAction.js")).default(args, context)
}, mediaGroup = {
  name: "media",
  signature: "[COMMAND]",
...
`, TEMPLATES = [{
  ...
  action: async (args, context) => {
    ...
    }), sluggedName = deburr(title.toLowerCase()).replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""), destDir = path.join(workDir, MIGRATIONS_DIRECTORY, sluggedName);
    if (existsSync(destDir) && !await prompt.single({
      type: "confirm",
    ...
    await writeFile(definitionFile, renderedTemplate), output.print(), output.print(`${chalk.green("\u2713")} Migration created!`), output.print(), output.print("Next steps:"), output.print(`Open ${chalk.bold(definitionFile)} in your code editor and write the code for your migration.`), output.print(`Dry run the migration with:
...
function resolveMigrationScript(workDir, migrationName) {
  return [migrationName, path.join(migrationName, "index")].flatMap((location) => MIGRATION_SCRIPT_EXTENSIONS.map((ext) => {
    const relativePath = path.join(MIGRATIONS_DIRECTORY, `${location}.${ext}`), absolutePath = path.resolve(workDir, relativePath);
    let mod;
    try {
      mod = require$1(absolutePath);
    } catch (err) {
...
function isLoadableMigrationScript(script) {
  if (typeof script.mod > "u" || !isPlainObject(script.mod.default))
    return !1;
...
async function resolveMigrations(workDir) {
  ...
  const migrationsDir = path.join(workDir, MIGRATIONS_DIRECTORY), migrationEntries = await readdir(migrationsDir, {
    withFileTypes: !0
  }), migrations = [];
...
function removeMigrationScriptExtension(fileName) {
  return MIGRATION_SCRIPT_EXTENSIONS.reduce((name, ext) => name.endsWith(`.${ext}`) ? path.basename(name, `.${ext}`) : name, fileName);
}
var migrationGroup = {
  name: "migration",
  signature: "[COMMAND]",
...
function convertToTree(nodes) {
  ...
  function addNode(node, tree = root) {
    if (!node.path.length) {
      tree.nodes || (tree.nodes = []), tree.nodes.push(node);
      return;
    }
    const [current, ...rest] = node.path, key = pathToString([current]);
...
function parseCliFlags(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("dry-run", {
    type: "boolean",
    default: !0
  }).options("concurrency", {
    type: "number",
...
const runMigrationCommand = {
  ...
  action: async (args, context) => {
    const {
      ...
      chalk,
      workDir
    } = context, [id] = args.argsWithoutOptions, migrationsDirectoryPath = path.join(workDir, MIGRATIONS_DIRECTORY), flags = await parseCliFlags(args), fromExport = flags.fromExport, dry = flags.dryRun, dataset = flags.dataset, project = flags.project, apiVersion = flags.apiVersion;
    ...
    if (!id) {
      ...
      const migrations = await resolveMigrations(workDir), table = new Table({
 ...
 - `)}`);
    ...
    if (concurrency !== void 0) {
      if (concurrency > MAX_MUTATION_CONCURRENCY)
        throw new Error(`Concurrency exceeds the maximum allowed value of ${MAX_MUTATION_CONCURRENCY}`);
      if (concurrency === 0)
    ...
    const actualDataset = dataset || (projectConfig.dataset === "~dummy-placeholder-dataset-" ? void 0 : projectConfig.dataset);
    if (!actualDataset)
      throw new Error('sanity.cli.js does not contain a dataset name ("api.dataset") and no --dataset option was provided.');
    const apiConfig = {
...
async function getPreviewAction$1() {
  return (await import("./previewAction.js")).default;
}
...
`, deleteSchemaCommand = {
  ...
  helpText: helpText$a,
  action: async (args, context) => {
    const result = await (await import("./deleteSchemaAction.js")).default(args.extOptions, context);
...
`, deploySchemaCommand = {
  ...
  helpText: helpText$9,
  action: async (args, context) => {
    const result = await (await import("./deploySchemasAction.js")).default(args.extOptions, context);
    return result === "failure" && process.exit(1), result;
  }
...
`, extractSchemaCommand = {
  ...
  helpText: helpText$8,
  action: async (args, context) => (await import("./extractAction.js")).default(args, context)
};
var schemaGroup = {
  name: "schema",
  signature: "[COMMAND]",
...
`, fetchSchemaCommand = {
  ...
  helpText: helpText$7,
  action: async (args, context) => {
    const result = await (await import("./listSchemasAction.js")).default(args.extOptions, context);
    return result === "failure" && process.exit(1), result;
  }
...
`, validateDocumentsCommand = {
  ...
  description,
  helpText: helpText$6,
  action: async (args, context) => (await import("./validateAction.js")).default(args, context)
}, isInteractive = process.stdout.isTTY && process.env.TERM !== "dumb" && !("CI" in process.env);
createRequire(import.meta.url);
const helpText$5 = `
Notes
...
`, startCommand = {
  ...
  action: async (args, context) => {
    const {
      ...
      chalk,
      prompt
    } = context, previewAction = await getPreviewAction(), error = (msg) => output.warn(chalk.red.bgBlack(msg));
    try {
...
async function deleteToken(specifiedToken, flags, context) {
  ...
  try {
    ...
    throw err.statusCode === 404 ? new Error(`Token with ID "${tokenId}" not found`, {
      cause: err
    }) : err;
  }
}
...
`, listUsersCommand = {
  ...
  action: async (args, context) => {
    ...
    })).filter((member) => !member.isRobot || robots), ...pendingInvitations], ordered = sortBy(members.map(({
      id,
      name,
      role,
      date
    }) => [id, name, role, date]), [sortFields.indexOf(sort)]), rows = order === "asc" ? ordered : ordered.reverse(), maxWidths = rows.reduce((max, row) => row.map((current, index) => Math.max(size(current), max[index])), sortFields.map((str) => size(str))), printRow = (row) => {
