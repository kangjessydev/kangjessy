                            ...
                            <div class="flex flex-1 overflow-hidden">
                                ...
                                <div
                                    ...
                                    <button @click="selectFolder(null)"
                                        ...
                                        :class="selectedFolderId === null ? 'bg-[#702DFF] text-white' : 'text-slate-500 hover:bg-slate-100'">
                                        <LayoutGrid :size="16" />
                                        <span>All Files</span>
                                    </button>
                                    ...
                                    <div class="space-y-1">
                                        <p
                                            ...
                                            Folders</p>
                                        <FolderItem v-for="folder in rootFolders" :key="folder.id" :folder="folder"
                                            :all-folders="folders" :selected-id="selectedFolderId" :level="0"
                                            @select="selectFolder" />
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 flex flex-col bg-white">
                                    ...
                                    <div
                                        ...
                                        <div class="relative flex-1 max-w-md">
                                            ...
                                            <input v-model="searchQuery" type="text" placeholder="Search media..."
                                    ...
                                    <!-- Grid Content -->
                                    <div class="flex-1 overflow-y-auto p-6 bg-[#F4F7FE]/50 relative">
                                        <!-- Loading State -->
                                        <div v-if="loading"
                                        ...
                                        <div v-if="filteredMedia.length > 0"
                                            ...
                                            <div v-for="item in filteredMedia" :key="item.id" @click="selectMedia(item)"
                                                class="group relative aspect-square rounded-2xl border-2 overflow-hidden cursor-pointer transition-all hover:shadow-lg"
                                                :class="selectedMedia?.id === item.id ? 'border-[#702DFF] ring-2 ring-[#702DFF]/20' : 'border-transparent bg-white'">
                                                <img :src="item.url"
                                                    class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />

                                                <!-- Selected Checkmark -->
                                                <div v-if="selectedMedia?.id === item.id"
                                                ...
                                                <div
                                                    ...
                                                    <p class="text-white/70 text-[10px]">{{ formatSize(item.size) }}</p>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div v-if="selectedMedia"
                                    ...
                                    <div class="space-y-4 flex-1">
                                        <div>
                                            <label
                                                class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">File
                                                Name</label>
                                            <p class="text-xs font-bold text-[#1B2559] break-all">{{ selectedMedia.name
                                                }}</p>
                                        </div>

                                        <div>
                                            <label
                                                class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Alt
                                                Text (SEO)</label>
                                            <textarea v-model="selectedMedia.alt" rows="3"
                                                ...
                                                placeholder="Deskripsikan gambar ini untuk SEO..."></textarea>
                                            <p class="text-[8px] text-slate-400 mt-1 font-bold italic uppercase">Penting
                                                untuk aksesibilitas</p>
                                        </div>

                                        <button @click="saveAltText" :disabled="savingAlt"
                                    ...
                                    <div class="pt-4 border-t border-slate-100 mt-auto flex flex-col gap-2">
                                        <button @click="confirmSelection"
                                            class="btn btn-primary w-full py-3 rounded-xl text-sm justify-center shadow-lg shadow-indigo-500/20">
                                            Insert Selected
                                        </button>
                                        <p class="text-[9px] text-slate-400 font-bold uppercase text-center mt-2">{{
                                            formatSize(selectedMedia.size) }} • {{
                                            selectedMedia.type?.split('/')[1]?.toUpperCase() }}</p>
                                    </div>
                                </div>
                            </div>
...
import { X, Search, Upload, LayoutGrid, Image, Loader2, Check } from 'lucide-vue-next'
...
onMounted(async () => {
    ...
    folders.value = [
        { id: 'blog', name: 'Blog', parentId: null, createdAt: new Date().toISOString() },
        { id: 'blog-covers', name: 'Covers', parentId: 'blog', createdAt: new Date().toISOString() },
        { id: 'blog-content', name: 'Content Images', parentId: 'blog', createdAt: new Date().toISOString() },
        { id: 'portfolio', name: 'Portfolio', parentId: null, createdAt: new Date().toISOString() },
        { id: 'portfolio-thumbnails', name: 'Thumbnails', parentId: 'portfolio', createdAt: new Date().toISOString() },
        { id: 'portfolio-galleries', name: 'Galleries', parentId: 'portfolio', createdAt: new Date().toISOString() },
        { id: 'products', name: 'Products', parentId: null, createdAt: new Date().toISOString() },
        { id: 'branding', name: 'Branding', parentId: null, createdAt: new Date().toISOString() },
        { id: 'misc', name: 'Misc', parentId: null, createdAt: new Date().toISOString() },
    ]
})
...
const saveAltText = async () => {
    ...
    try {
        await sanityWriteClient.patch(selectedMedia.value.id)
