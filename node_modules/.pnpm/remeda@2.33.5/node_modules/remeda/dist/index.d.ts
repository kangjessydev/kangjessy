...
@category Object
*/
type SimplifyDeep<Type, ExcludeType = never> = ConditionalSimplifyDeep<Type, ExcludeType | NonRecursiveType | MapsSetsOrArrays, object>;
//#endregion
...
*/
type InternalIsUnion<T, U = T> = (IsNever<T> extends true ? false : T extends any ? [U] extends [T] ? false : true : never) extends infer Result // In some cases `Result` will return `false | true` which is `boolean`,
...
@category Union
*/
type AllUnionFields<Union> = Extract<Union, NonRecursiveType | ReadonlyMap<unknown, unknown> | ReadonlySet<unknown> | UnknownArray> extends infer SkippedMembers ? Exclude<Union, SkippedMembers> extends infer RelevantMembers ? SkippedMembers | Simplify< // Include fields that are common in all union members
...
@category Template literal
*/
type CamelCase<Type, Options extends CamelCaseOptions$1 = {}> = Type extends string ? string extends Type ? Type : Uncapitalize<CamelCaseFromArray<Words<Type extends Uppercase<Type> ? Lowercase<Type> : Type, Options>, ApplyDefaultOptions<CamelCaseOptions$1, _DefaultCamelCaseOptions, Options>>> : Type;
//#endregion
...
type TuplePartsWithoutFixed<T extends IterableContainer, Optional extends unknown[] = []> = T extends readonly [(infer Head)?, ...infer Tail] ? Or<T extends readonly [] ? true : false, T[number][] extends Tail ? true : false> extends true ? {
  /**
...
type IsLongerThan$1<T extends readonly unknown[], N extends number> = IsEqual$1<T[N], undefined> extends true ? false : true;
...
type SwapArray<T extends IterableContainer, K1 extends number, K2 extends number> = And<IsNonNegative<K1>, IsNonNegative<K2>> extends true ? And<isLessThan<K1, T["length"]>, isLessThan<K2, T["length"]>> extends true ? SwapArrayInternal<T, K1, K2> : T : T[number][];
