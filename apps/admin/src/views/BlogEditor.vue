  ...
  <div class="page-container" id="blog-editor-page">
    ...
    >
      <div class="flex flex-col gap-1">
        ...
        >
          <router-link to="/blog" class="hover:text-[#702DFF] transition-colors"
            >Blog Articles</router-link
          >
          <ChevronRight :size="10" />
          <span class="text-[#702DFF]">{{
            isEdit ? "Edit Post" : "New Article"
          }}</span>
        </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
      ...
      <div class="lg:col-span-3 space-y-6" id="main-editor-column">
        ...
        <div class="space-y-5 px-1" id="header-section">
          ...
          <div
            class="flex items-center gap-2 px-3 py-1 bg-white border border-slate-100/50 rounded-xl w-fit shadow-sm"
            id="slug-indicator"
          >
            <span
        ...
        >
          <div class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <div class="flex items-center justify-between mb-2 px-2">
                  ...
                  >
                  <span
                    :class="getTitleHint(post.seo_title || '').color"
                    class="text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider"
                  >
                ...
                <BaseInput
                  ...
                  placeholder="Best title for Google..."
                  id="input-seo-title"
                />
                <div
                  class="mt-2 h-1 w-full bg-slate-100 rounded-full overflow-hidden mx-2"
                >
                  <div
                    class="h-full transition-all duration-500"
                    :class="getTitleHint(post.seo_title || '').barColor"
                    :style="{
                      width:
                        Math.min(
                          ((post.seo_title?.length || 0) / 60) * 100,
                          100,
                        ) + '%',
                    }"
                  ></div>
                ...
                <p
                  class="text-[9px] text-slate-400 mt-1.5 font-bold uppercase tracking-tighter mx-2"
                >
                  {{ post.seo_title?.length || 0 }} / 60 Characters
                </p>
              </div>
              <div>
                <label
                ...
                <div class="relative">
                  <div
                    class="input-field min-h-[42px] flex flex-wrap gap-2 py-2 pr-10 transition-all"
                    :class="{
                      'border-[#702DFF] ring-4 ring-[#702DFF]/5':
                        isKeywordsFocused,
                    }"
                  >
                    <span
                    ...
                    <input
                      ...
                      type="text"
                      class="bg-transparent border-none p-0 text-sm focus:ring-0 flex-1 min-w-[100px]"
                ...
                <p class="text-[9px] text-slate-400 mt-1.5 font-bold uppercase">
                  Press Enter or Comma to add (First one is the Focus Keyword)
                </p>
              </div>
            </div>
            ...
            <!-- Article Excerpt -->
            <div
              class="mb-8 shadow-sm bg-indigo-50/20 p-5 rounded-2xl border border-indigo-100/50"
            >
              <div class="flex items-center justify-between mb-4 px-2">
                <label
                  class="text-[10px] font-black text-slate-400 uppercase tracking-widest block"
                  >Article Excerpt (Tampil di List)</label
                >
                <div class="flex items-center gap-3">
                  <button
                    @click="regenerateExcerpt"
                    class="text-[9px] font-black text-[#702DFF] hover:underline uppercase tracking-tighter"
                  >
                    Regenerate
                  </button>
                  <span
                    class="text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider bg-white text-slate-400 border border-slate-100"
                  >
              ...
              />
              <p
                class="text-[9px] text-slate-400 mt-2 font-bold uppercase italic tracking-tighter px-2"
              >
                Diambil dari konten jika kosong. Ringkasan ini muncul di halaman
                depan.
              </p>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-2 px-2">
                ...
                >
                <span
                  :class="getDescHint(post.seo_description || '').color"
                  class="text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider"
                >
              ...
              >
                <div
                  class="h-full transition-all duration-500"
                  :class="getDescHint(post.seo_description || '').barColor"
                  :style="{
                    width:
                      Math.min(
                        ((post.seo_description?.length || 0) / 160) * 100,
                        100,
                      ) + '%',
                  }"
                ></div>
              ...
              <p
                class="text-[9px] text-slate-400 mt-1.5 font-bold uppercase tracking-tighter mx-2"
              >
                {{ post.seo_description?.length || 0 }} / 160 Characters
              </p>
            </div>
            ...
            <div class="pt-6 border-t border-slate-100 mt-6 overflow-hidden">
              <div class="flex items-center justify-between mb-5">
                <div
                  class="flex items-center gap-1 p-1 bg-slate-100/50 rounded-lg"
                >
                  <button
                    @click="activeAnalysisTab = 'seo'"
                  ...
                  <button
                    @click="activeAnalysisTab = 'readability'"
                    class="px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all"
                    :class="
                  ...
                  <button
                    @click="activeAnalysisTab = 'social'"
                    class="px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all"
                    :class="
                      activeAnalysisTab === 'social'
                        ? 'bg-white text-[#702DFF] shadow-sm'
                        : 'text-slate-400 hover:text-slate-600'
                    "
                  >
                  ...
                  <div
                    ...
                    :class="
                      activeAnalysisTab === 'seo'
                        ? overallSeoScore.bg
                        : activeAnalysisTab === 'readability'
                          ? overallReadabilityScore.bg
                          : 'bg-blue-500'
                    "
                  ></div>
                  <span
                    class="text-[9px] font-bold px-2 py-1 rounded-full uppercase tracking-widest"
                    :class="
                      activeAnalysisTab === 'seo'
                        ? overallSeoScore.class
                        : activeAnalysisTab === 'readability'
                          ? overallReadabilityScore.class
              ...
              <!-- SEO Tab Content -->
              <div
                v-show="activeAnalysisTab === 'seo'"
                class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3"
              >
                <div
                  v-for="(check, idx) in seoChecks"
                  :key="idx"
                  class="flex items-start gap-3 group"
                >
                  <div
                    class="mt-1 shrink-0 transition-transform group-hover:scale-110"
                  >
                    <div
                      v-if="check.status === 'good'"
                      class="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]"
                    ></div>
                    <div
                      v-else-if="check.status === 'ok'"
                      class="w-3 h-3 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.4)]"
                    ></div>
                    <div
                      v-else
                      class="w-3 h-3 rounded-full bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)]"
                    ></div>
                  </div>
                  <div class="flex flex-col">
                    <p
                      class="text-[11px] font-bold text-slate-600 line-clamp-1 group-hover:text-[#1B2559] transition-colors"
                    >
                      {{ check.title }}
                    </p>
                    <p
                      class="text-[9px] font-medium text-slate-400 leading-tight"
                    >
                      {{ check.message }}
                    </p>
                  </div>
              ...
              <div
                v-show="activeAnalysisTab === 'readability'"
                class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3"
              >
                <div
                  v-for="(check, idx) in readabilityChecks"
                  :key="idx"
                  class="flex items-start gap-3 group"
                >
                  <div
              ...
              <div v-show="activeAnalysisTab === 'social'" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="space-y-4">
                    <BaseInput
                      v-model="post.social_title"
                      label="Social Title"
                      :placeholder="post.seo_title || post.title || ''"
                    />
                    <AdminTextarea
                      v-model="post.social_description"
                  ...
                  <div class="space-y-4">
                    ...
                    <div
                      class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm max-w-[320px]"
                    >
                      <div
                        class="aspect-[1.91/1] bg-slate-100 flex items-center justify-center relative overflow-hidden group"
                      >
                        <img
                          v-if="post.social_image || post.featured_image"
                          :src="post.social_image || post.featured_image"
                        ...
                        >
                          <ImageIcon :size="24" />
                          <span class="text-[8px] font-bold uppercase mt-1"
                            >Image 1.91:1</span
                          >
                        </div>
                      </div>
                      <div class="p-3 bg-[#F2F3F5] border-t border-slate-200">
                        <p
                          class="text-[10px] text-slate-500 uppercase font-black tracking-tighter"
                        >
                          kangjessy.com
                        </p>
                        <h6
                          class="text-[13px] font-bold text-[#1c1e21] leading-[1.3] mt-1 line-clamp-1"
                        >
                          {{
                            post.social_title || post.seo_title || post.title
                          }}
                        </h6>
                        <p
                          class="text-[11px] text-[#606770] leading-[1.3] mt-1 line-clamp-2"
                        >
                          {{
            ...
            <!-- SEO Preview -->
            <div
              class="p-5 bg-slate-50/50 rounded-[20px] border border-slate-100/50 mt-8"
              id="google-preview"
            >
              <p
                class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3"
              >
                Google Preview
              </p>
              <h4
                class="text-lg font-bold text-[#1a0dab] hover:underline cursor-pointer mb-1"
              >
                {{ post.seo_title || post.title || "Your Article Title" }}
              </h4>
              <p class="text-xs text-[#006621] mb-1.5 font-medium">
                kangjessy.com/blog/{{ post.slug || "slug" }}
              </p>
              <p class="text-[11px] text-slate-500 leading-relaxed">
                {{
                  post.seo_description ||
                  post.excerpt ||
                  "Write a meta description to see how it looks on Google results page..."
                }}
              </p>
            </div>
          </div>
      ...
      <div class="lg:col-span-1 space-y-6" id="sidebar-editor-column">
        ...
        >
          <div class="space-y-5">
            ...
            <div v-if="post.status === 'scheduled'">
              <label
                class="text-[10px] font-black text-[#A3AED0] uppercase tracking-widest block mb-2"
                >Schedule Date</label
              >
              <input
                v-model="post.publish_schedule"
                type="datetime-local"
        ...
        >
          <div class="space-y-6">
            ...
            <div>
              <label
                class="text-[10px] font-black text-[#A3AED0] uppercase tracking-widest block mb-3"
                >Categories</label
              >
              <div
                class="space-y-1.5 max-h-40 overflow-y-auto pr-2 custom-scrollbar"
              >
                <label
                ...
                >
                  ...
                  />
                  <span
                    class="text-[11px] font-bold text-slate-600 group-hover:text-[#1B2559]"
                    >{{ cat.name }}</span
                  >
                </label>
              </div>
            ...
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  @keyup.enter="addTag"
                  type="text"
                  class="input-field !py-2.5 text-[11px]! !pl-9"
              ...
              <div v-if="suggestedTags.length > 0" class="mt-3">
                <p
                  class="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-2"
                >
                  Suggested
                </p>
                <div class="flex flex-wrap gap-1.5">
                  <button
                    v-for="tag in suggestedTags.filter(
                      (t) => !post.tags?.includes(t.id),
                    )"
                    :key="tag.id"
        ...
        >
          <div class="space-y-4">
            <div
              class="aspect-[16/10] bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 overflow-hidden relative group cursor-pointer transition-all hover:border-indigo-200 hover:bg-slate-100/50"
              @click="openMediaPicker"
              id="image-picker-zone"
            >
              <!-- Image Preview -->
              <img
              ...
              <!-- Placeholder -->
              <div
                v-if="!post.featured_image && !uploading"
                class="absolute inset-0 flex flex-col items-center justify-center text-slate-400 gap-2"
              >
                <div
                  class="w-12 h-12 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-300 group-hover:text-[#702DFF] transition-colors"
                >
                  <ImageIcon :size="20" />
                </div>
                <div class="text-center">
                  ...
                  >
                  <span
                    class="block text-[8px] font-bold text-slate-400 mt-1 uppercase"
                    >Recommended 16:10</span
                  >
                </div>
              </div>
            ...
            <div v-if="post.featured_image" class="space-y-2">
              <label
                ...
                >Alt Text (Deskripsi Gambar)</label
              >
              <input
                v-model="post.featured_image_alt"
                type="text"
                class="input-field !py-2.5 text-[10px]!"
                placeholder="Deskripsikan gambar ini untuk SEO..."
                id="input-featured-image-alt"
              />
              <p class="text-[8px] text-slate-300 font-bold uppercase italic">
                Penting untuk aksesibilitas & SEO Google
              </p>
            </div>
...
const keywordTags = computed(() => {
  ...
  return post.value.seo_keywords
    .split(",")
    .map((k) => k.trim())
    .filter((k) => k);
});
...
const addKeyword = () => {
  ...
  const currentKeywords = post.value.seo_keywords
    ? post.value.seo_keywords.split(",").map((k) => k.trim())
    : [];
...
const seoChecks = computed(() => {
  ...
  if (!focusKW) {
    checks.push({
      title: "Focus Keyword",
      message: "Tentukan kata kunci fokus untuk memulai analisis.",
      status: "bad",
    });
  } else {
    ...
    if (intro.includes(focusKW)) {
      checks.push({
        title: "Keyword in Intro",
        message: "Keren! Kata kunci ditemukan di paragraf pertama.",
        status: "good",
      });
    } else {
      checks.push({
        title: "Keyword in Intro",
        message: "Coba tambahkan kata kunci di 100 kata pertama.",
        status: "ok",
      });
    }
    ...
    if (title.includes(focusKW)) {
      checks.push({
        ...
        message: "Sempurna! Kata kunci Anda ada di dalam judul.",
        status: "good",
      });
    } else {
      checks.push({
        title: "Keyword in Title",
        message: "Coba masukkan kata kunci fokus ke dalam judul.",
        status: "bad",
      });
    }

    // Keyword in H2/H3
    const hasInHeading = (post.value.content || "")
      .match(/<h[23][^>]*>.*?<\/h[23]>/gi)
      ?.some((h: any) => h.toLowerCase().includes(focusKW));
    if (hasInHeading) {
      checks.push({
        title: "Keyword in Subheading",
        message: "Bagus sekali! Kata kunci ditemukan di H2/H3.",
        status: "good",
      });
    } else {
      checks.push({
        title: "Keyword in Subheading",
        message: "Gunakan kata kunci di dalam judul sub-bab (H2/H3).",
        status: "ok",
      });
    }
    ...
    if (desc.includes(focusKW)) {
      checks.push({
        ...
        message: "Kerja bagus, kata kunci ada di deskripsi.",
        status: "good",
      });
    } else {
      checks.push({
        title: "Keyword in Meta Description",
        message: "Tambahkan kata kunci ke dalam deskripsi meta.",
        status: "ok",
      });
    }
    ...
    if (slug.includes(focusKW.replace(/\s+/g, "-"))) {
      checks.push({
        ...
        message: "URL slug Anda sudah teroptimasi dengan baik.",
        status: "good",
      });
    } else {
      checks.push({
        title: "Keyword in Slug",
        message: "Slug URL sebaiknya mengandung kata kunci.",
        status: "ok",
      });
    }
    ...
    if (count >= 2) {
      checks.push({
        ...
        message: `Ditemukan ${count} kali. Sangat bagus!`,
        status: "good",
      });
    } else if (count === 1) {
      checks.push({
        title: "Keyword Density",
        message: "Kata kunci hanya ditemukan sekali di konten.",
        status: "ok",
      });
    } else {
      checks.push({
        title: "Keyword Density",
        message: "Kata kunci tidak ditemukan di dalam isi konten.",
        status: "bad",
      });
    }
  }
  ...
  const hasExternalLink = (post.value.content || "").match(
    /<a[^+]+href="http(s)?:\/\/(?!kangjessy\.com)/i,
  );

  checks.push({
    title: "Internal Links",
    message: hasInternalLink
      ? "Mantap, ditemukan link internal."
      : "Belum ada link internal ke website Anda.",
    status: hasInternalLink ? "good" : "ok",
  });
  checks.push({
    title: "External Links",
    message: hasExternalLink
      ? "Bagus, ada link referensi keluar."
      : "Belum ada link keluar (outbound links).",
    status: hasExternalLink ? "good" : "ok",
  });

  // 6. Image Alt Text
  const imagesInContent = (post.value.content || "").match(/<img[^>]+>/g) || [];
  const missingAltInContent = imagesInContent.some(
    (img: any) => !img.includes("alt=") || img.includes('alt=""'),
  );
  ...
  if (imagesInContent.length > 0) {
    checks.push({
      ...
      message: missingAltInContent
        ? "Ditemukan gambar di konten tanpa Alt Text."
        : "Semua gambar di konten sudah punya Alt Text.",
      status: missingAltInContent ? "bad" : "good",
    });
  }
  ...
  if (post.value.featured_image) {
    checks.push({
      ...
      message: hasFeaturedAlt
        ? "Featured image sudah punya Alt Text."
        : "Featured image belum punya Alt Text.",
      status: hasFeaturedAlt ? "good" : "bad",
    });
  }
  ...
  if (content.length > 500) {
    checks.push({
      title: "Text Length",
      message: "Panjang artikel sudah sangat ideal.",
      status: "good",
    });
  } else if (content.length > 200) {
    checks.push({
      title: "Text Length",
      message: "Sedikit pendek, coba tambah detail lagi.",
      status: "ok",
    });
  } else {
    checks.push({
      title: "Text Length",
      message: "Konten terlalu tipis untuk bersaing.",
      status: "bad",
    });
  }
  ...
  if (hasPlaceholder) {
    checks.push({
      title: "Quality Check",
      message: "Peringatan: Ditemukan teks placeholder (Ipsum)!",
      status: "bad",
    });
  }
  ...
  checks.push({
    ...
    message:
      titleStatus === "Perfect"
        ? "Panjang judul sudah optimal."
        : "Panjang judul perlu disesuaikan.",
    status:
      titleStatus === "Perfect"
        ? "good"
        : titleStatus === "Missing"
...
const overallSeoScore = computed(() => {
  ...
  if (
    total === 0 ||
    (total === 1 && firstCheck && firstCheck.title === "Focus Keyword")
  ) {
    return {
      text: "Empty",
...
const readabilityChecks = computed(() => {
  ...
  const longSentences = sentences.filter(
    (s) => s.trim().split(/\s+/).length > 20,
  );
  const percentageLong =
    sentences.length > 0 ? (longSentences.length / sentences.length) * 100 : 0;

  if (percentageLong < 20) {
    checks.push({
      title: "Sentence length",
      message: "Bagus! Kalimat Anda singkat dan padat.",
      status: "good",
    });
  } else {
    checks.push({
      title: "Sentence length",
      message: `${Math.round(percentageLong)}% kalimat > 20 kata. Usahakan lebih pendek.`,
      status: "bad",
    });
  }
  ...
  const longParagraphs = paragraphs.filter(
    (p) => p.replace(/<[^>]*>/g, "").split(/\s+/).length > 150,
  );
  if (longParagraphs.length === 0 && paragraphs.length > 0) {
    checks.push({
      title: "Paragraph length",
      message: "Tidak ada paragraf yang terlalu panjang.",
      status: "good",
    });
  } else if (paragraphs.length > 0) {
    checks.push({
      title: "Paragraph length",
      message: "Coba potong paragraf yang terlalu panjang.",
      status: "bad",
    });
  }
  ...
  const passiveCount = sentences.filter((s) => {
    ...
    return words.some(
      (w) =>
        w.startsWith("di") || w.startsWith("ter") || passiveWords.includes(w),
    );
  }).length;
  ...
  if (passivePercentage < 10) {
    checks.push({
      title: "Passive voice",
      message: "Penggunaan kalimat aktif sangat baik.",
      status: "good",
    });
  } else {
    checks.push({
      title: "Passive voice",
      message: `${Math.round(passivePercentage)}% kalimat pasif. Gunakan lebih banyak kalimat aktif.`,
      status: "ok",
    });
  }
  ...
  if (hasTransitions) {
    checks.push({
      ...
      message: "Penggunaan kata penghubung sudah baik.",
      status: "good",
    });
  } else {
    checks.push({
      title: "Transition words",
      message: "Gunakan lebih banyak kata penghubung (namun, karena, dll).",
      status: "ok",
    });
  }
  ...
  for (let i = 0; i < sentences.length - 1; i++) {
    const firstWord1 = (
      sentences[i]?.trim().split(/\s+/)[0] || ""
    ).toLowerCase();
    const firstWord2 = (
      sentences[i + 1]?.trim().split(/\s+/)[0] || ""
    ).toLowerCase();
    if (firstWord1 && firstWord1 === firstWord2 && firstWord1.length > 3) {
      consecutiveSame = true;
      break;
    }
  }
  if (!consecutiveSame) {
    checks.push({
      ...
      message: "Variasi awal kalimat sangat bagus.",
      status: "good",
    });
  } else {
    checks.push({
      title: "Sentence variety",
      message: "Hindari memulai kalimat dengan kata yang sama berturut-turut.",
      status: "ok",
    });
  }
...
const overallReadabilityScore = computed(() => {
  const good = readabilityChecks.value.filter(
    (c) => c.status === "good",
  ).length;
...
const addTag = async () => {
  ...
  const existing = suggestedTags.value.find(
...
const handleSave = async (redirect: boolean = false) => {
  ...
  try {
    ...
    showToast(
      `Save failed: ${errorMsg}. Please check your connection.`,
      "error",
    );
  } finally {
    saving.value = false;
