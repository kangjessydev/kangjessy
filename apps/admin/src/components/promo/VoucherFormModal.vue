                        ...
                        <DialogPanel
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 font-medium mt-0.5">Configure discounts and usage
                                        limits</p>
                                </div>
                            ...
                            <div class="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Voucher
                                            Code</label>
                                        <div class="relative">
                                            <Tag :size="16"
                                                class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                                            <input v-model="formData.code" type="text" placeholder="e.g. SALE2026"
                                    ...
                                    <div class="space-y-2">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Label
                                            (Public)</label>
                                        <input v-model="formData.label" type="text" placeholder="e.g. New Year Promo"
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea v-model="formData.desc" rows="2"
                                        placeholder="Describe the promo details..."
                                        class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-medium text-[#1B2559] focus:outline-none focus:ring-2 focus:ring-[#702DFF]/20 focus:border-[#702DFF] resize-none"></textarea>
                                </div>

                                <!-- Discount Rules -->
                                <div class="p-5 border border-slate-100 rounded-2xl bg-slate-50/30 space-y-4">
                                    <p class="text-xs font-black text-[#702DFF] flex items-center gap-2">
                                        <Percent :size="14" />
                                        Discount Rules
                                    </p>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div class="space-y-2">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Type</label>
                                        ...
                                        <div class="space-y-2 md:col-span-2">
                                            ...
                                            <div class="relative">
                                                <span v-if="formData.type === 'fixed'"
                                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">Rp</span>
                                                <span v-else
                                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">%</span>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div class="space-y-2">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expiry
                                                Date</label>
                                            <div class="relative">
                                                <Calendar :size="16"
                                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                                                <input v-model="formData.expiry_date" type="date"
                                        ...
                                        <div class="space-y-2">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Min.
                                                Order (Optional)</label>
                                            <div class="relative">
                                                <span
                                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">Rp</span>
                                                <input v-model.number="formData.min_order" type="number" placeholder="0"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <!-- Status Toggle -->
                                    <div class="flex items-center justify-between p-4 border border-slate-100 rounded-xl bg-white hover:border-[#702DFF] transition-colors cursor-pointer"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-colors"
                                                :class="formData.is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'">
                                                <Power :size="20" />
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-[#1B2559]">Active Status</p>
                                                <p class="text-[10px] text-slate-400 font-medium">Enable usage</p>
                                            </div>
                                        </div>
                                        <div class="w-12 h-6 rounded-full p-1 transition-colors relative"
                                            :class="formData.is_active ? 'bg-emerald-500' : 'bg-slate-200'">
                                            <div class="w-4 h-4 rounded-full bg-white shadow-sm transition-all"
                                                :class="formData.is_active ? 'translate-x-6' : 'translate-x-0'"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between p-4 border border-slate-100 rounded-xl bg-white hover:border-[#702DFF] transition-colors cursor-pointer"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-colors"
                                                :class="formData.is_global ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'">
                                                <Globe :size="20" />
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-[#1B2559]">Global Voucher</p>
                                                <p class="text-[10px] text-slate-400 font-medium">Valid for all items
                                                </p>
                                            </div>
                                        </div>
                                        <div class="w-12 h-6 rounded-full p-1 transition-colors relative"
                                            :class="formData.is_global ? 'bg-[#702DFF]' : 'bg-slate-200'">
                                            <div class="w-4 h-4 rounded-full bg-white shadow-sm transition-all"
                                                :class="formData.is_global ? 'translate-x-6' : 'translate-x-0'"></div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Footer -->
                            <div class="px-8 py-5 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/50">
                                <button @click="closeModal"
