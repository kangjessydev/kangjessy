<template>
    <div class="bg-bg-secondary border rounded-3xl p-6 sm:p-8 lg:p-10 flex flex-col justify-between transition-[border-color,background-color,box-shadow] duration-500 relative h-full group overflow-hidden"
        :class="[
            service.isFeatured ? 'border-accent-primary/50 shadow-[0_0_30px_rgba(var(--accent-rgb),0.15)] bg-bg-secondary/80 backdrop-blur-sm' : 'border-border-color hover:border-accent-primary/30',
        ...
        <div class="flex justify-between items-center mb-6 sm:mb-10 relative z-10">
            ...
            <div class="flex flex-col items-end gap-1">
                <span
                    ...
                    :class="{ 'bg-accent-primary! text-white! border-accent-primary!': service.isFeatured }">
                    {{ service.status === 'coming-soon' ? 'Coming Soon' : (service.badge || 'Professional Solution') }}
                </span>
            </div>
        </div>

        <div class="flex-1 relative z-10">
            <h3 class="text-[clamp(1.75rem,3vw,2rem)] font-black mb-4 text-text-primary leading-[1.15] tracking-tight">
                {{ service.title }}
            </h3>
            <p
            ...
            <!-- Features List -->
            <div v-if="service.included" class="space-y-4"
                :class="{ 'opacity-50 grayscale': service.status === 'coming-soon' }">
                <div v-for="f in service.included.slice(0, 4)" :key="f"
                    class="flex items-center gap-3.5 text-[clamp(0.85rem,1.5vw,0.9rem)] text-text-secondary font-medium">
                    <div class="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                        :class="service.isFeatured ? 'bg-accent-primary text-white' : 'bg-accent-primary/10 text-accent-primary'">
                        <CheckIcon :size="12" />
                    </div>
                    <span>{{ f }}</span>
                </div>
        ...
        <div class="mt-12 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 relative z-10">
            <div class="flex flex-col gap-1.5">
                ...
                <!-- Pricing Container -->
                <div class="flex items-center gap-2 relative group/price cursor-default">
                    ...
                    <!-- Hot Deal Flame Icon -->
                    <div v-if="pricing.original && pricing.original > pricing.display && service.status !== 'coming-soon'"
                        ...
                        <Flame :size="18" class="transition-transform duration-700 group-hover/price:animate-pulse"
                            stroke-width="2.5" />
                        ...
                        <div
                            ...
                            Promo Hari Ini
                            <div class="absolute top-full left-3 -mt-1 border-4 border-transparent border-t-amber-500">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <BaseButton :to="service.status === 'coming-soon' ? '#' : `/service/${service.id}`"
                :variant="getButtonVariant(service)" size="lg" :disabled="service.status === 'coming-soon'"
                class="w-full sm:w-auto rounded-xl! px-8! group/btn"
                :class="{ 'opacity-70 cursor-not-allowed': service.status === 'coming-soon' }">
                <span
                    :class="service.status === 'coming-soon' ? 'text-text-primary font-bold' : 'text-white font-bold tracking-wide'">
                    {{ service.status === 'coming-soon' ? 'Coming Soon' : 'Lihat Solusi' }}
                </span>
                <ArrowRightIcon :size="18" v-if="service.status !== 'coming-soon'"
...
import type { Service } from '../../data/landing/services';
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const pricing = computed(() => {
            ...
            const relatedPackages = projectTypes.filter(p => p.serviceId === props.service.id && p.basePrice > 0);
            ...
            const sortedByPrice = [...relatedPackages].sort((a, b) => a.basePrice - b.basePrice);
