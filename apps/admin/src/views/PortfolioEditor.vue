<template>
    <div class="page-container" id="portfolio-editor-page">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex flex-col gap-1">
                <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#B5B5B5]">
                    <router-link to="/portfolio" class="hover:text-[#702DFF] transition-colors">Portfolio
                        Showcase</router-link>
                    <ChevronRight :size="10" />
                    <span class="text-[#702DFF]">{{ isEdit ? 'Edit Case Study' : 'New Case Study' }}</span>
                </nav>
                <h1 class="text-2xl font-black text-[#1B2559] mt-1">{{ headerTitle }}</h1>
            </div>
        ...
        <transition enter-active-class="transition duration-300 ease-out"
            enter-from-class="translate-y-20 opacity-0 scale-90" enter-to-class="translate-y-0 opacity-100 scale-100"
            ...
            <div v-if="isScrolled" class="fixed bottom-8 right-8 z-[100] flex flex-col gap-3">
                <button @click="handleSave"
                    ...
                    :class="{ 'opacity-80 pointer-events-none': saving }" title="Save Changes">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3 space-y-6">
                ...
                <div class="space-y-4 px-1">
                    <div>
                        <input v-model="project.title" @input="handleTitleInput" type="text"
                    ...
                    <div
                        ...
                        <span class="text-[9px] font-black text-[#A3AED0] uppercase tracking-widest">SLUG:</span>
                        <span v-if="!isEditingSlug" class="text-[11px] font-bold text-[#702DFF]">{{ project.slug ||
                            'generating...' }}</span>
                        <input v-else v-model="project.slug" type="text"
                ...
                <div class="flex gap-1 bg-slate-100 p-1 rounded-2xl w-fit">
                    <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id"
                        ...
                        :class="activeTab === tab.id ? 'bg-white text-[#702DFF] shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                        {{ tab.name }}
                    </button>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div v-show="activeTab === 'general'" class="space-y-6">
                        <DashboardCard title="Ringkasan & Link">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="md:col-span-2">
                                    <label class="label-heading">Ringkasan Pendek (Muncul di Kartu Portofolio)</label>
                                    <textarea v-model="project.excerpt" rows="3" class="input-field"
                                        placeholder="Misal: Website profil perusahaan dengan sistem manajemen konten otomatis dan desain premium..."></textarea>
                                </div>
                                <div>
                                    <label class="label-heading">Link Preview (Live / Demo)</label>
                                    <div class="relative">
                                        <Globe :size="16"
                                            class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                                        <input v-model="project.websiteUrl" type="url" class="input-field !pl-12"
                                            placeholder="https://..." />
                                    </div>
                                </div>
                                <div>
                                    <label class="label-heading">Kode Sumber (GitHub)</label>
                                    <div class="relative">
                                        <Github :size="16"
                                            class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                                        <input v-model="project.githubUrl" type="url" class="input-field !pl-12"
                        ...
                        <div class="space-y-3">
                            <label class="label-heading !pl-1">Latar Belakang Proyek (Fase 01)</label>
                            <TipTapEditor :model-value="getChallengeText()" @update:model-value="updateChallenge" />
                        </div>
                    </div>
                    ...
                    <div v-show="activeTab === 'process'" class="space-y-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div v-for="(item, idx) in project.pivot.items" :key="idx"
                                        ...
                                        <div
                                            ...
                                            <input v-model="item.icon" type="text" class="w-full bg-transparent border-none text-center text-xs font-bold p-0
                                                focus:ring-0" placeholder="Icon" />
                                            <span
                                                class="text-[8px] text-slate-300 font-black uppercase mt-0.5">Icon</span>
                                        </div>
                                        <div class="flex-1 space-y-2">
                                            <input v-model="item.title" type="text"
                                                class="w-full bg-transparent border-none font-bold text-sm p-0 focus:ring-0"
                                                placeholder="Judul Temuan" />
                                            <textarea v-model="item.desc" rows="2"
                                                class="w-full bg-transparent border-none text-xs text-slate-500 p-0 focus:ring-0 resize-none"
                                                placeholder="Description..."></textarea>
                                        </div>
                                        <button @click="project.pivot.items.splice(idx, 1)"
                                    ...
                                    <button @click="project.pivot.items.push({ icon: 'Search', title: '', desc: '' })"
                        ...
                        <!-- Solution Steps -->
                        <DashboardCard v-if="project.steps" title="Fase 02 / Strategi (Solusi)">
                            <div class="space-y-4">
                                <div v-for="(step, idx) in project.steps" :key="idx"
                                    ...
                                    <div class="flex-1 space-y-2">
                                        ...
                                        <textarea v-model="step.desc" rows="3"
                                            class="w-full bg-transparent border-none text-sm text-slate-500 p-0 focus:ring-0 resize-none"
                                            placeholder="How it was implemented..."></textarea>
                                    </div>
                                    <button @click="project.steps.splice(idx, 1)"
                                        class="absolute -right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 w-8 h-8 rounded-full bg-white shadow-md text-rose-500 flex items-center justify-center transition-all hover:scale-110">
                                        <X :size="14" />
                                    </button>
                                </div>
                                <button @click="project.steps.push({ title: '', desc: '' })"
                    ...
                    <div v-show="activeTab === 'impact'" class="space-y-6">
                        <DashboardCard v-if="project.metrics" title="Fase 03 / Dampak (Hasil)">
                            <div class="space-y-6">
                                <div>
                                    <label class="label-heading">Hasil Akhir / Testimonial</label>
                                    <textarea v-model="project.impact" rows="3" class="input-field italic"
                                        placeholder="e.g. 'Project was delivered ahead of schedule with 99.9% uptime...'"></textarea>
                                </div>

                                <div class="space-y-3">
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest !pl-1">
                                        Metrik Utama & Bukti Hasil</p>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div v-for="(metric, idx) in project.metrics" :key="idx"
                                            ...
                                            <div class="flex gap-4">
                                                <div
                                                    ...
                                                    <input v-model="metric.icon" type="text"
                                            ...
                                            <div class="mt-4 flex flex-col gap-2">
                                                <label
                                                    class="text-[8px] font-black text-slate-400 uppercase tracking-widest pl-1">Proof
                                                    Image URL (Optional)</label>
                                                <input v-model="metric.proofImage" type="text"
                                            ...
                                            <button @click="project.metrics.splice(idx, 1)"
                                        ...
                                        <button
                                            @click="project.metrics.push({ icon: 'TrendingUp', value: '', label: '' })"
                    ...
                    <div v-show="activeTab === 'media'" class="space-y-6">
                        <DashboardCard v-if="project.gallery" title="Project Gallery (Showcase)">
                            <div class="space-y-4">
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div v-for="(img, idx) in project.gallery" :key="idx"
                                        ...
                                        <button @click="project.gallery.splice(idx, 1)"
                                    ...
                                    <button @click="openMediaPickerForGallery"
                                        ...
                                        <ImageIcon :size="24" class="mb-2 group-hover:scale-110 transition-transform" />
                        ...
                        <DashboardCard v-if="project.faqs" title="F.A.Q (Frequently Asked Questions)">
                            <div class="space-y-4">
                                <div v-for="(faq, idx) in project.faqs" :key="idx"
                                    ...
                                    <div class="flex-1 space-y-3">
                                        <input v-model="faq.q" type="text"
                                            class="w-full bg-transparent border-none font-bold text-sm p-0 focus:ring-0"
                                            placeholder="Question Title" />
                                        <textarea v-model="faq.a" rows="3"
                                            class="w-full bg-transparent border-none text-xs text-slate-500 p-0 focus:ring-0 resize-none border-t border-slate-200 pt-2"
                                            placeholder="Detailed answer..."></textarea>
                                    </div>
                                    <button @click="project.faqs.splice(idx, 1)"
                                ...
                                <button @click="project.faqs.push({ q: '', a: '' })"
                                    class="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 text-[10px] font-black uppercase tracking-widest hover:border-[#702DFF] hover:text-[#702DFF] transition-all">
                                    + Tambah F.A.Q
                                </button>
                            </div>
                        ...
                        <div class="space-y-3">
                            <label class="label-heading pl-1">Konten Detail / Case Study Tambahan (Muncul di Bawah
                                FAQ)</label>
                            <TipTapEditor ref="editorRef" :model-value="project.content || ''"
            ...
            <div class="lg:col-span-1 space-y-6 pt-16">
                ...
                <DashboardCard title="Status & Visibilitas" :stretch="false">
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="relative" ref="dropdownRef">
                                <button @click="toggleDropdown"
                                    ...
                                    <span class="text-xs font-bold"
                                        :class="getCategoryName(project.category) ? 'text-[#1B2559]' : 'text-slate-400'">
                                        {{ getCategoryName(project.category) || 'Select Area' }}
                                    </span>
                                    <ChevronRight :size="14" class="text-slate-400 transition-transform duration-200"
                                        :class="{ 'rotate-90': isAreaDropdownOpen }" />
                                </button>

                                <!-- Dropdown Menu -->
                                <div v-if="isAreaDropdownOpen"
                                    ...
                                    <div class="p-2 border-b border-slate-50 bg-white sticky top-0 z-10">
                                        <div class="relative">
                                            <Search :size="12"
                                                class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" />
                                            <input v-model="areaInput" ref="areaSearchInput"
                                                @keyup.enter="handleAreaEnter" type="text"
                                    ...
                                    <div class="max-h-48 overflow-y-auto custom-scrollbar p-1">
                                        <button v-for="area in filteredAreas" :key="area.id"
                                            ...
                                            <span
                                                :class="getCategoryName(project.category) === area.name ? 'text-[#702DFF]' : 'text-slate-600'">{{
                                                    area.name }}</span>
                                            <Check v-if="getCategoryName(project.category) === area.name" :size="12"
                                                class="text-[#702DFF]" />
                                        </button>

                                        <!-- Add New Option -->
                                        <button
                                            v-if="areaInput && !filteredAreas.some(a => a.name.toLowerCase() === areaInput.toLowerCase())"
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex flex-col">
                                                    <span class="text-[9px] font-black uppercase text-[#702DFF]">Tambah
                                                        Area
                                                        Baru</span>
                                        ...
                                        <!-- Empty State -->
                                        <div v-if="!areaInput && filteredAreas.length === 0" class="py-4 text-center">
                                            <p class="text-[9px] font-bold text-slate-300 uppercase tracking-widest">
                                                Area tidak ditemukan</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex bg-slate-50 p-1 rounded-xl">
                                <button @click="project.status = 'SUCCESS'"
                                    class="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all"
                                    :class="project.status === 'SUCCESS' ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'">
                                    Success
                                </button>
                                <button @click="project.status = 'IDEA'"
                                    class="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all"
                                    :class="project.status === 'IDEA' ? 'bg-indigo-500 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'">
                                    Idea
                                </button>
                            </div>
                        </div>
                        <div class="pt-2 border-t border-slate-100">
                            <label class="label-heading">Visibilitas di Web</label>
                            <select v-model="project.visibility" class="select-field !py-2.5 !text-xs">
                ...
                <!-- Tech Stack (Detailed) -->
                <DashboardCard title="Teknologi Inti" :stretch="false">
                    <div class="space-y-3">
                        ...
                        <div class="relative">
                            <input v-model="techInput" @keyup.enter="addTechTag" type="text"
                                class="input-field !py-2 !text-[10px] !pl-9" placeholder="Tambah teknologi..." />
                            <Code :size="12" class="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-300" />
                        </div>
                        <div v-if="availableTechnologies.length > 0" class="mt-2">
                            <div class="flex flex-wrap gap-1 max-h-32 overflow-y-auto custom-scrollbar">
                                <button
                                    v-for="tech in availableTechnologies.filter(t => !project.technologies?.includes(t.id))"
                                    :key="tech.id" @click="addSuggestedTech(tech)"
                ...
                <!-- Client Info -->
                <DashboardCard title="Metadata Konteks" :stretch="false">
                    <div class="space-y-4">
                        <div>
                            <label class="label-heading">Nama Klien</label>
                            <div class="relative">
                                <User :size="14" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" />
                                <input v-model="project.client" type="text" class="input-field !py-2 !pl-10 !text-xs"
                                    placeholder="e.g. Nur Alam Hotel" />
                            </div>
                        </div>
                        <div>
                            <label class="label-heading">Industri</label>
                            <div class="relative">
                                <Briefcase :size="14" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" />
                                <input v-model="project.industry" type="text" class="input-field !py-2 !pl-10 !text-xs"
                                    placeholder="e.g. Hospitality" />
                            </div>
                        </div>
                        <div>
                            <label class="label-heading">Tanggal Penyelesaian</label>
                            <input v-model="project.completionDate" type="date" class="input-field !py-2 !text-xs" />
                        </div>
                    </div>
                </DashboardCard>

                <!-- Featured Image -->
                <DashboardCard title="Thumbnail Utama" :stretch="false">
                    <div class="space-y-4">
                        <div class="aspect-[16/10] bg-slate-50 rounded-[32px] border-2 border-dashed border-slate-200 overflow-hidden relative group cursor-pointer transition-all hover:border-[#702DFF] hover:bg-slate-100/50"
                            @click="openMediaPicker">
                            <img v-if="project.mainImage" :src="project.mainImage" class="w-full h-full object-cover" />
                            ...
                            <div v-if="!project.mainImage"
                                ...
                                <div class="space-y-1.5 relative z-10">
                                    <span class="text-xs font-black uppercase tracking-[0.2em] text-[#1B2559]">Pilih
                                        Foto Utama</span>
                                    <p
                                        class="text-[9px] text-slate-400 uppercase font-black tracking-widest opacity-60">
                                        Rekomendasi: 16:10 Ratio / PNG / JPG</p>
                                </div>
                            ...
                            <!-- Remove Button -->
                            <button v-if="project.mainImage"
                                @click.stop="project.mainImage = ''; project.mainImageRef = ''"
        ...
        <MediaPickerModal :is-open="isMediaPickerOpen" @close="isMediaPickerOpen = false" @select="handleMediaSelect" />
    </div>
</template>
...
import {
    ChevronRight, Save, Edit3, Check, User, Globe, Github,
    Image as ImageIcon, X, Plus, Code, Briefcase, Search,
    Camera, UploadCloud
...
const headerTitle = computed(() => isEdit.value ? "Modify Case Study" : "Create New Case Study")
...
onMounted(async () => {
    ...
    if (isEdit.value) {
        try {
            ...
            areaInput.value = typeof project.value.category === 'string'
...
const handleTitleInput = () => {
    if (!isEditingSlug.value && !isEdit.value) {
        project.value.slug = project.value.title?.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '')
    }
}
...
const handleAreaEnter = async () => {
    ...
    const existing = availableAreas.value.find(a => a.name.toLowerCase() === val.toLowerCase())
    if (existing) {
...
const addTechTag = async () => {
    ...
    if (!val) return
    techInput.value = ''
    const existing = availableTechnologies.value.find(t => t.name.toLowerCase() === val.toLowerCase())
    if (existing) {
        addSuggestedTech(existing)
    } else {
        try {
            const newTech = await portfolioService.createTechnology({ name: val, slug: val.toLowerCase().replace(/\s+/g, '-') })
            availableTechnologies.value.push(newTech)
            addSuggestedTech(newTech)
        } catch (e) { showToast('Failed to create tech', 'error') }
    }
}

const addSuggestedTech = (tech: Technology) => {
    if (!project.value.technologies) project.value.technologies = []
    if (!project.value.technologies.includes(tech.id)) project.value.technologies.push(tech.id)
}

const removeTag = (id: string) => {
    project.value.technologies = project.value.technologies?.filter(t => t !== id)
}

const getTagName = (id: string) => availableTechnologies.value.find(t => t.id === id)?.name || id
...
const handleSave = async () => {
    ...
    if (typeof payload.category === 'string') {
        const matchedArea = availableAreas.value.find(
            a => a.name.toLowerCase() === (payload.category as string).toLowerCase()
        )
        if (matchedArea) {
            // @ts-ignore
            payload.category = {
