<template>
  <div class="page-container max-w-7xl mx-auto pb-24 relative px-4 sm:px-6 lg:px-8">
    <!-- Background Decor -->
    <div class="absolute top-0 right-0 -z-10 w-[500px] h-[500px] bg-indigo-100/30 blur-[120px] rounded-full opacity-50">
    </div>
    <div class="absolute bottom-0 left-0 -z-10 w-[400px] h-[400px] bg-blue-50/40 blur-[100px] rounded-full opacity-40">
    </div>
    <!-- Header -->
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 class="text-3xl lg:text-4xl font-black text-[#1B2559] leading-tight tracking-tight drop-shadow-sm">{{
            pageTitle }}</h1>
        </div>
      </div>

      <div class="flex items-center gap-3">
        <BaseButton v-if="isEdit" variant="secondary" @click="openWhatsAppModal">
          <MessageCircle :size="18" />
          Share Update
        </BaseButton>
        <div class="flex items-center bg-white border border-slate-100 rounded-2xl p-1 gap-1 shadow-sm">
          <BaseButton variant="secondary" :loading="saving && !isClosing" @click="handleSave(false)"
          ...
          <BaseButton variant="primary" :loading="saving && isClosing" @click="handleSave(true)"
    ...
    <div v-if="loading"
      ...
      <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-slate-50 border-t-[#702DFF]"></div>
      <p class="mt-4 text-slate-400 font-bold uppercase tracking-widest text-[10px]">Assembling Project Boards...</p>
    </div>
    ...
    <template v-else>
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-8">
          <!-- Project Context -->
          <DashboardCard title="Project Foundation"
            class="shadow-2xl shadow-indigo-500/5 bg-white/70 backdrop-blur-xl !rounded-[40px] border-none"
            :stretch="false">
            <div class="space-y-6">
              <div class="space-y-1.5">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Project Active
                  Name</label>
                <BaseInput v-model="formData.name" placeholder="Acme Inc - Web Revamp" :icon="Layout" />
              </div>

              <div class="relative group">
                <select v-model="formData.order_id" @change="onOrderSelect"
                  class="select-field group-hover:border-indigo-100 transition-all !text-[11px] !font-black !py-3">
                  <option :value="undefined">No Linked Order (Internal/Friend)</option>
              ...
              <p v-if="!isEdit" class="text-[9px] text-slate-300 font-bold uppercase mt-1">Note: Only shows DEAL orders
                that aren't converted yet.</p>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Start
                    Projection</label>
                  <BaseInput v-model="formData.start_date" type="date" :icon="Calendar" />
                </div>
                <div class="space-y-1.5">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Target
                    Deadline</label>
                  <BaseInput v-model="formData.deadline" type="date" :icon="Clock" />
                </div>
              </div>
            </div>
          </DashboardCard>
          ...
          <DashboardCard title="Deliverables & Assets" :stretch="false"
            class="shadow-2xl shadow-indigo-500/5 bg-white/70 backdrop-blur-xl !rounded-[40px] border-none">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-1.5">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Figma/Design
                  Link</label>
                <BaseInput v-model="formData.figma_url" placeholder="https://figma.com/..." />
              </div>
              <div class="space-y-1.5">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">GitHub
                  Repository</label>
                <BaseInput v-model="formData.github_url" placeholder="https://github.com/..." />
              </div>
              <div class="md:col-span-2 space-y-6 pt-4 border-t border-slate-50 mt-2">
                <div class="space-y-1.5">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Core Brief &
                    Objective</label>
                  <textarea v-model="formData.brief" class="input-field min-h-[100px] pt-3"
                    placeholder="What is the main goal and requirements for this project?"></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Visual Style &
                      Mood</label>
                    <textarea v-model="formData.visual_style" class="input-field min-h-[80px] pt-3"
                      placeholder="Premium, minimal, dark mode, high contrast, typography focused..."></textarea>
                  </div>
                  <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Timeline &
                      Delivery Notes</label>
                    <textarea v-model="formData.timeline_notes" class="input-field min-h-[80px] pt-3"
                      placeholder="Milestone deadlines, specific meeting schedules..."></textarea>
                  </div>
                </div>
              </div>
            </div>
          ...
          <DashboardCard title="Production Roadmap" :stretch="false"
              ...
              <div class="flex items-center gap-4">
                <div class="relative group/tpl">
                  ...
                  <div
                    ...
                    <div class="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-3 px-2">Roadmap
                      Presets
                    </div>
                    <button v-for="tpl in projectTemplates" :key="tpl.name" @click="applyTemplate(tpl)"
                      ...
                      <p class="text-xs font-black text-[#1B2559] group-hover:text-[#7029FF] line-clamp-1">{{ tpl.name
                        }}</p>
                      <p class="text-[9px] font-medium text-slate-400 mt-0.5">{{ tpl.description }}</p>
                    </button>
                  </div>
                </div>
            ...
            <div class="space-y-8">
              ...
              <div class="flex gap-2 p-4 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl">
                <input v-model="newPhaseName" type="text" placeholder="Add new Task List (e.g. BRIEF, DESIGN, DEV)..."
                  class="input-field !py-3 flex-1 !bg-white" @keyup.enter="addPhase" />
                <BaseButton variant="secondary" @click="addPhase" :disabled="!newPhaseName.trim()">
                  <Layers :size="18" />
                  Add Milestone
                </BaseButton>
              </div>
              ...
              <div v-for="(phaseTasks, phaseName) in groupedTasks" :key="phaseName" class="space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                  <div class="flex items-center gap-3">
                    <!-- Collapse Toggle -->
                    <button @click="togglePhase(phaseName)"
                      class="w-6 h-6 flex items-center justify-center rounded-md hover:bg-slate-100 text-slate-400 transition-all">
                      <ChevronDown v-if="!isPhaseCollapsed(phaseName)" :size="14" />
                      <ChevronRight v-else :size="14" />
                    </button>

                    <!-- Icon Picker -->
                    <div class="relative">
                      <button @click.stop="activeIconPicker = activeIconPicker === phaseName ? null : phaseName"
                      ...
                      <!-- Icon Dropdown -->
                      <div v-if="activeIconPicker === phaseName"
                        ...
                        <button v-for="(icon, icon_name) in availableIcons" :key="icon_name"
                    ...
                    <div class="flex-1 min-w-0">
                      <input v-if="editingPhase === phaseName" v-model="phaseEditName" @blur="savePhaseName(phaseName)"
                        ...
                        v-focus />
                      <h4 v-else @click="startEditingPhase(phaseName)"
                      ...
                      <div class="flex items-center gap-1.5 mt-0.5">
                        <div class="w-16 h-1 bg-slate-100 rounded-full overflow-hidden">
                          <div class="h-full bg-[#7029FF] rounded-full transition-all"
                            :style="{ width: getPhaseProgress(phaseTasks) + '%' }"></div>
                        </div>
                ...
                <div v-show="!isPhaseCollapsed(phaseName)" class="space-y-3 pl-10 relative">
                  <!-- Hierarchy Line -->
                  <div class="absolute left-[27px] top-0 bottom-10 w-px bg-slate-100 pointer-events-none"></div>
                  ...
                  <div v-for="task in phaseTasks" :key="task.id"
                    ...
                    <button @click.stop="toggleTask(task)"
                      ...
                      :class="isTaskCompleted(task) ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 hover:border-[#7029FF] hover:shadow-lg hover:shadow-indigo-500/10'">
                      <Check :size="12" v-if="isTaskCompleted(task)" stroke-width="4" />
                    </button>

                    <div class="flex-1 min-w-0">
                      <input v-if="editingTask === task.id" v-model="taskEditTitle" @blur="saveTaskTitle(task)"
                        @keyup.enter="saveTaskTitle(task)"
                        class="text-[11px] font-bold text-[#7029FF] bg-transparent outline-none w-full" v-focus />
                      <p v-else @click="startEditingTask(task)"
                        class="text-[11px] font-bold text-[#1B2559] line-clamp-2 cursor-text hover:text-[#7029FF] transition-colors"
                        :class="{ 'line-through text-slate-400': isTaskCompleted(task) }">
                        {{ task.title }}
                      </p>
                    </div>
                  ...
                  <!-- Quick Add Task -->
                  <div class="relative flex items-center gap-3 p-1.5 transition-all">
                    <div
                      class="absolute -left-[13px] top-1/2 -translate-y-1/2 w-[13px] h-px bg-slate-100 pointer-events-none">
                    </div>
                    <div
                    ...
                    <input v-model="newTaskInPhases[phaseName]" @keyup.enter="addTaskToPhase(phaseName)" type="text"
                    ...
                    <button @click="addTaskToPhase(phaseName)" :disabled="!newTaskInPhases[phaseName]?.trim()"
              ...
              <div v-if="tasks.length === 0"
                ...
                <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1">Board is empty</p>
                <p class="text-[11px] font-bold text-slate-400">Add milestones like BRIEF, UI/UX, and DEV to start
                  tracking.</p>
              </div>
            </div>
          </DashboardCard>
        ...
        <div class="lg:col-span-4 space-y-6">
          <DashboardCard title="Lifecycle & Meta" :stretch="false"
            class="shadow-2xl shadow-indigo-500/5 bg-white/70 backdrop-blur-xl !rounded-[40px] border-none">
            <div class="space-y-6">
              <div>
                <label
                  class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Status</label>
              ...
              <div>
                <label class="flex justify-between items-center mb-2">
                  <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Overall
                    Progress</span>
                  <span class="text-sm font-black text-[#7029FF]">{{ autoProgress }}%</span>
                </label>
                <div class="h-4 bg-slate-50 rounded-full border border-slate-100 overflow-hidden shadow-inner p-1">
                  <div
                ...
                <p class="text-[9px] font-black text-slate-300 uppercase mt-2 tracking-widest text-center">
                  CALCULATED FROM {{ tasks.length }} TASKS
                </p>
              </div>

              <div class="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl relative overflow-hidden group/token">
                <p class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-2">Portal Token</p>
                <div class="flex items-center justify-between">
                  <div class="flex flex-col">
                    <code
                      class="text-xs font-black text-[#7029FF]">{{ formData.client_portal_token || 'GENERATING...' }}</code>
                    <p v-if="!formData.client_portal_token"
                      class="text-[8px] font-bold text-indigo-300 uppercase mt-0.5">Will
                      be saved on next update</p>
                  </div>
                  <button @click="copyPortalLink"
                    ...
                    :class="copied ? 'text-emerald-500 bg-white' : 'text-slate-400 hover:text-[#7029FF]'">
      ...
      <WhatsAppModal :is-open="waModal.isOpen" :text="waModal.text" :client-name="currentClientName"
        :client-phone="currentClientPhone" @close="waModal.isOpen = false" />

      <!-- Confirmation Modal -->
      <ConfirmModal :is-open="confirmModal.isOpen" :title="confirmModal.title" :message="confirmModal.message"
        variant="danger" @close="confirmModal.isOpen = false" @confirm="executeDelete" />

      <!-- Template Confirmation Modal -->
      <ConfirmModal :is-open="templateConfirmModal.isOpen" title="Apply Template?"
        message="This will add template milestones to your current roadmap. Existing tasks will not be deleted. Continue?"
        confirm-text="Apply Template" @close="templateConfirmModal.isOpen = false"
...
import {
  ArrowLeft, Save, ChevronRight, Layout, Clock, Calendar,
  Plus, Trash2, Check, MessageCircle, Layers, Wand2,
  ChevronUp, ChevronDown, Code, Smartphone, Globe, Database,
  Rocket, Search, CheckCircle, FileText, Settings, Zap
} from 'lucide-vue-next'

const availableIcons: Record<string, any> = {
  Layers, Layout, Code, Smartphone, Globe, Database,
  Rocket, Search, CheckCircle, FileText, Settings, Zap
}
...
const pageTitle = computed(() => isEdit.value ? 'Modify Production Board' : 'Initiate New Project')
...
onMounted(async () => {
  ...
  window.addEventListener('click', () => { activeIconPicker.value = null })
})

function ensurePortalToken() {
  if (!formData.value.client_portal_token) {
    formData.value.client_portal_token = Math.random().toString(36).substring(2, 15)
  }
}
...
async function executeApplyTemplate(template: ProjectTemplate) {
  ...
  loading.value = true
  try {
    const isNewProject = !isEdit.value || id.value?.toString().startsWith('temp')
...
const completedTasksCount = computed(() => tasks.value.filter(t => isTaskCompleted(t)).length)
...
const groupedTasks = computed(() => {
  ...
  const sortedTasks = [...tasks.value].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
...
function addPhase() {
  ...
  if (groupedTasks.value[name]) return showToast('Milestone already exists', 'error')
...
async function savePhaseName(oldName: string) {
  if (!editingPhase.value || !phaseEditName.value.trim() || phaseEditName.value.toUpperCase() === oldName) {
  ...
  if (isEdit.value && !id.value?.toString().startsWith('temp')) {
    try {
      const tasksInPhase = tasks.value.filter(t => t.phase === newName && !t.id.toString().startsWith('temp'))
...
async function addTaskToPhase(phaseName: string) {
  ...
  const maxSort = tasks.value.length > 0 ? Math.max(...tasks.value.map(t => t.sort_order || 0)) : 0
  ...
  if (isEdit.value && !id.value?.toString().startsWith('temp')) {
    try {
      ...
      console.error(err)
      showToast(`Failed to save task: ${err.message || 'Unknown error'}`, 'error')
    }
  } else {
    // For new projects
    const tempTask = { ...newTask, id: 'temp-' + Date.now() + Math.random(), is_completed: false } as any
...
async function saveTaskTitle(task: any) {
  if (!editingTask.value || !taskEditTitle.value.trim() || taskEditTitle.value === task.title) {
...
async function setPhaseIcon(phaseName: string, iconName: string) {
  ...
  if (isEdit.value && !id.value?.toString().startsWith('temp')) {
    const phaseTasks = tasks.value.filter(t => t.phase === phaseName && !t.id.toString().startsWith('temp'))
    try {
    ...
    // Just update locally
    tasks.value.forEach(t => { if (t.phase === phaseName) t.icon = iconName })
  }
}
...
async function moveTask(task: any, direction: 'up' | 'down') {
  ...
  const swapTask = direction === 'up' ? phaseTasks[taskIdxInPhase - 1] : phaseTasks[taskIdxInPhase + 1]
...
async function toggleTask(task: any) {
  try {
    ...
    const index = tasks.value.findIndex(t => t.id?.toString() === task.id?.toString())
    if (index === -1) return
...
async function deleteTask(taskId: string) {
  ...
  confirmModal.message = 'Are you sure you want to remove this task from the list?'
...
function onOrderSelect() {
  const order = availableOrders.value.find(o => o.id === formData.value.order_id)
  if (order) {
...
async function handleSave(shouldClose: boolean = true) {
  if (!formData.value.name) return showToast('Project Name is required', 'error')
  saving.value = true
  ...
  try {
    ...
    if (isEdit.value && !id.value.toString().startsWith('temp')) {
      ...
      const tempTasks = tasks.value.filter(t => t.id?.toString().startsWith('temp'))
...
function openWhatsAppModal() {
  ...
  const status = (formData.value.status || 'new').toUpperCase().replace('_', ' ')
...
const currentClientName = computed(() => (formData.value as any).client?.name || 'Client')
const currentClientPhone = computed(() => (formData.value as any).client?.phone || '')
</script>
