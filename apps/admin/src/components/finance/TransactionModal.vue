          ...
          >
            ...
            >
              <div class="flex items-center justify-between mb-8">
                <div>
                  <h3 class="text-xl font-black text-[#1B2559] leading-tight">
                    Record Entry
                  </h3>
                  <p
                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1"
                  >
                    Movement of capital record
                  </p>
                </div>
                <button
                  @click="closeModal"
              ...
              <div class="space-y-6">
                <!-- Type Toggle -->
                <div
                  class="grid grid-cols-2 lg:grid-cols-4 gap-1 p-1 bg-slate-50 rounded-2xl border border-slate-100"
                >
                  <button
                    @click="formData.type = 'income'"
                    class="py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5"
                    :class="
                  ...
                  <button
                    @click="formData.type = 'receivable'"
                    class="py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5"
                    :class="
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    <span
                      class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 font-bold text-xs pointer-events-none"
                      >Rp</span
                    >
                    <input
                      type="text"
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div class="space-y-2">
                    ...
                    >
                      ...
                      <optgroup
                        label="Expense"
                        v-else-if="formData.type === 'expense'"
                      >
                      ...
                      <optgroup
                        label="Piutang (Klien Utang Kita)"
                        v-else-if="formData.type === 'receivable'"
                      >
                        <option value="unpaid_invoice">Unpaid Invoice</option>
                        <option value="loan_to_staff">Staff Loan</option>
                      </optgroup>
                      <optgroup label="Hutang (Kita Utang Orang)" v-else>
                        <option value="bank_loan">Bank/External Loan</option>
                        <option value="vendor_debt">Vendor Debt</option>
                ...
                <div class="space-y-2">
                  ...
                  <input
                    ...
                    type="text"
                    placeholder="e.g. DP Web App Development"
...
const formData = ref({
  type: "income" as "income" | "expense" | "debt" | "receivable",
  amount: 0,
  category: "services",
...
const filteredProjects = computed(() => {
  ...
  return allProjects.value.filter(
    (p) => p.client_id === formData.value.client_id,
  );
});
...
watch(
  ...
  (newType) => {
    formData.value.sub_category =
      newType === "income" ? "project_dp" : "hosting";
    formData.value.category = newType === "income" ? "services" : "other";
  },
);

const formattedAmount = computed(() =>
  formData.value.amount ? formData.value.amount.toLocaleString("id-ID") : "",
);
