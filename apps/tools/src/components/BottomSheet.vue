    ...
    <Teleport to="body">
        ...
        <Transition enter-active-class="transition-opacity duration-200"
            ...
            <div v-if="modelValue" @click="handleBackdropClick"
                class="fixed inset-0 bg-black/80 backdrop-blur-md z-10000" :class="backdropClass"></div>
        </Transition>

        <!-- Bottom Sheet / Side Drawer Panel -->
        <Transition :name="isDesktop ? 'drawer' : 'sheet'">
            <div v-if="modelValue" ref="sheetRef" @touchstart="handleTouchStart" @touchmove="handleTouchMove"
                ...
                :class="[
                    sheetClass,
                    // Shared & Mobile Initial
                    'bottom-0 left-0 right-0 border-t rounded-t-[32px]',
                    { 'h-[75vh]': !localFullHeight, 'h-[97vh] border-t-0 rounded-t-none': localFullHeight },
                    // Desktop Overrides
                ...
                <!-- Header -->
                <div v-if="title || $slots.header" class="px-6 py-4 border-b border-border-color/50">
                    <slot name="header">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <h4 class="text-[1.1rem] font-bold text-text-primary tracking-tight">{{ title }}</h4>
                            </div>
                            <button v-if="showClose" @click="emit('update:modelValue', false)"
                ...
                <!-- Content Area -->
                <div ref="contentRef" class="overflow-y-auto overflow-x-hidden flex-1 custom-scrollbar pb-20" :class="contentClass">
                    <slot></slot>
                </div>

                <!-- Footer (Optional) -->
                <div v-if="$slots.footer" class="border-t border-border-color bg-inherit">
...
// Sync localFullHeight with prop
watch(() => props.fullHeight, (newVal) => {
...
watch(() => props.modelValue, (newValue) => {
    sheetManager.syncWithModel(newValue);
    if (newValue) {
...
const handleTouchStart = (e: TouchEvent) => {
    ...
    if (touch) {
        ...
        startedOnHandle.value = handleRef.value?.contains(e.target as Node) || false;
    }
};
...
const handleTouchMove = (e: TouchEvent) => {
    ...
    // 1. Logic for Dragging Handle UP (Expand)
    ...
    // 2. Logic for Pulling Down to Close
    // Only close if we are at the top of the content or dragging the handle
