          ...
          <div class="modal-content">
            ...
            <div class="modal-header">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-indigo-100 text-[#702DFF] flex items-center justify-center">
                  <FileUp :size="20" />
                </div>
                <div>
                  <h3 class="text-sm font-black text-[#1B2559]">Import Data</h3>
                  <p class="text-[10px] text-slate-400">Restore from backup file</p>
                </div>
              </div>
            ...
            <div class="modal-body">
              <!-- No file selected -->
              <div v-if="!selectedFile" class="upload-zone" @click="triggerFileSelect" @dragover.prevent @drop.prevent="handleDrop">
                <div class="upload-icon">
                  <FileJson :size="32" />
                </div>
                <p class="text-xs font-bold text-slate-600 mt-3">Click to select or drag & drop</p>
              ...
              <div v-else-if="preview" class="preview-zone">
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <div class="flex-1">
                    <p class="text-xs font-bold text-[#1B2559]">{{ selectedFile.name }}</p>
                    <p class="text-[10px] text-slate-400">{{ formatFileSize(selectedFile.size) }}</p>
                  </div>
                  <button @click="clearFile" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                ...
                <div class="bg-slate-50 rounded-xl p-4 space-y-2">
                  <div class="flex justify-between text-xs">
                    ...
                    <span class="font-bold text-[#1B2559]">{{ preview.module }}</span>
                  </div>
                  <div class="flex justify-between text-xs">
                    <span class="text-slate-500">Records</span>
                    <span class="font-bold text-[#1B2559]">{{ preview.recordCount }}</span>
                  </div>
                  <div v-if="preview.meta?.exportedAt" class="flex justify-between text-xs">
                    <span class="text-slate-500">Exported</span>
                    <span class="font-bold text-slate-600">{{ formatDate(preview.meta.exportedAt) }}</span>
                  </div>
                </div>

                <!-- Warning -->
                <div class="flex items-start gap-2 mt-4 p-3 bg-amber-50 rounded-xl border border-amber-100">
                  <AlertTriangle :size="14" class="text-amber-500 shrink-0 mt-0.5" />
                  <p class="text-[10px] text-amber-700">Import will add new records. Existing data will not be affected.</p>
                </div>
              </div>

              <!-- Error state -->
              <div v-else-if="error" class="error-zone">
                <div class="w-12 h-12 rounded-xl bg-rose-100 text-rose-500 flex items-center justify-center mb-3">
                  <AlertCircle :size="24" />
                </div>
                <p class="text-xs font-bold text-rose-600">{{ error }}</p>
                <button @click="clearFile" class="text-[10px] text-slate-500 hover:text-slate-700 mt-2">Try another file</button>
              </div>
            </div>
...
import { Upload, Loader2, FileUp, X, FileJson, FileSpreadsheet, CheckCircle, AlertTriangle, AlertCircle } from 'lucide-vue-next'
