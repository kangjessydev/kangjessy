<template>
    <div class="page-container">
        <PageHeader title="Media Library" subtitle="Organize your assets with folders">
        ...
        <div class="flex gap-6">
                ...
                <DashboardCard title="Folders" :stretch="false">
                    <div class="space-y-1">
                        ...
                        <button @click="selectFolder(null)"
                            ...
                            :class="selectedFolderId === null ? 'bg-indigo-50 text-[#702DFF]' : 'hover:bg-slate-50 text-slate-600'">
                            <Database :size="16" />
                            <span class="text-[11px] font-bold flex-1">All Files</span>
                            <span class="text-[10px] font-black text-slate-400">{{ totalMediaCount }}</span>
                        </button>

                        <!-- Folder Tree -->
                        <div v-for="folder in rootFolders" :key="folder.id">
                            <FolderItem :folder="folder" :all-folders="allFolders" :selected-id="selectedFolderId"
                                :level="0" @select="selectFolder" @delete="handleDeleteFolder" />
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-slate-100">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Storage
                                Used</span>
                            <span class="text-[10px] font-bold text-[#702DFF]">{{ formatBytes(totalSize) }}</span>
                        </div>
                        <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div class="h-full bg-gradient-to-r from-[#702DFF] to-indigo-400 rounded-full"
                                :style="{ width: `${Math.min((totalSize / maxStorage) * 100, 100)}%` }"></div>
                        </div>
                        <p class="text-[9px] text-slate-400 mt-1 text-right">of {{ formatBytes(maxStorage) }}</p>
                    </div>
                </DashboardCard>
            </div>
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center gap-2 mb-6 text-[11px] font-bold">
                    <button @click="selectFolder(null)" class="text-slate-400 hover:text-[#702DFF] transition-colors">
                        All Files
                    </button>
                    <div v-for="(crumb, idx) in breadcrumbs" :key="crumb.id" class="flex items-center gap-2">
                        <ChevronRight :size="12" class="text-slate-300" />
                        <button @click="selectFolder(crumb.id)"
                            :class="idx === breadcrumbs.length - 1 ? 'text-[#702DFF]' : 'text-slate-400 hover:text-[#702DFF]'"
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex gap-2 bg-slate-100 p-1 rounded-xl">
                        <button @click="viewMode = 'grid'" class="p-2 rounded-lg transition-all"
                            :class="viewMode === 'grid' ? 'bg-white text-[#702DFF] shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                            <Grid3x3 :size="18" />
                        </button>
                        <button @click="viewMode = 'list'" class="p-2 rounded-lg transition-all"
                            :class="viewMode === 'list' ? 'bg-white text-[#702DFF] shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                            <List :size="18" />
                        </button>
                    </div>
                </div>
                ...
                <div v-if="currentSubfolders.length > 0" class="mb-6">
                    <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Folders</h3>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        <div v-for="subfolder in currentSubfolders" :key="subfolder.id"
                            ...
                            <p class="text-sm font-bold text-[#1B2559] truncate">{{ subfolder.name }}</p>
                            <p class="text-[10px] text-slate-400">{{ getFolderItemCount(subfolder.id) }} items</p>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="filteredMedia.length > 0">
                    <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Files</h3>
                    ...
                    <div v-if="viewMode === 'grid'"
                        ...
                        <div @click="triggerInput"
                            ...
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Upload</span>
                        </div>

                        <!-- Media Items -->
                        <div v-for="item in filteredMedia" :key="item.id" @click="selectedFile = item"
                            ...
                            :class="selectedFile?.id === item.id ? 'ring-4 ring-[#702DFF]/20 border-[#702DFF]' : ''">
                                ...
                                <tr v-for="item in filteredMedia" :key="item.id"
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-bold text-[#1B2559] truncate max-w-[200px]">{{
                                                item.name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 text-xs text-slate-500 hidden md:table-cell">{{
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center justify-end gap-1">
                                            <button @click="copyUrl(item.url)" class="btn-ghost w-8 h-8"
                                                title="Copy URL">
                                                <Copy :size="14" />
                                            </button>
                                            <button @click="openMoveModal(item)" class="btn-ghost w-8 h-8" title="Move">
                                                <FolderInput :size="14" />
                                            </button>
                ...
                <div v-else-if="!loading" class="flex flex-col items-center justify-center py-20">
                    ...
                    <p class="text-sm text-slate-400 mb-4">Upload files or create a subfolder</p>
                    <BaseButton variant="primary" @click="triggerInput">
                        <Upload :size="18" />
                        Upload Files
                    </BaseButton>
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
                ...
                <div v-if="showNewFolderModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="showNewFolderModal = false">
                    </div>
                    <div class="relative bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
                        <h2 class="text-xl font-black text-[#1B2559] mb-6">Create New Folder</h2>
                        <div class="space-y-4">
                            <div>
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Folder
                                    Name</label>
                                <input v-model="newFolderName" type="text" class="input-field"
                                    placeholder="e.g. Blog Images" autofocus />
                            </div>
                            <div>
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Parent
                                    Folder</label>
                        ...
                        <div class="flex gap-3 mt-8">
                            <BaseButton variant="secondary" class="flex-1" @click="showNewFolderModal = false">Cancel
                            </BaseButton>
                            <BaseButton variant="primary" class="flex-1" @click="createFolder">Create Folder
                            </BaseButton>
                        </div>
                    </div>
                </div>
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95"
                ...
                <div v-if="selectedFile" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    <!-- Backdrop -->
                    <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="selectedFile = null"></div>
                    
                    <!-- Modal Content -->
                    <div class="relative bg-white rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[85vh]">
                        
                        <!-- Left: Image Preview -->
                        <div class="w-full md:w-3/5 bg-slate-900 flex items-center justify-center relative group p-8">
                            <img :src="selectedFile.url" class="max-w-full max-h-full object-contain shadow-2xl rounded-lg" />
                            
                            <!-- Overlay Actions -->
                            <div class="absolute top-4 right-4 flex gap-2">
                                <button @click="copyUrl(selectedFile.url)" class="p-2 bg-white/10 hover:bg-white text-white hover:text-slate-900 rounded-lg backdrop-blur-md transition-all" title="Copy URL">
                                    <Copy :size="16" />
                                </button>
                                <a :href="selectedFile.url" target="_blank" class="p-2 bg-white/10 hover:bg-white text-white hover:text-slate-900 rounded-lg backdrop-blur-md transition-all" title="Open Original">
                        ...
                        <!-- Right: Details Form -->
                        <div class="w-full md:w-2/5 flex flex-col border-l border-slate-100 bg-white">
                            <!-- Header -->
                            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-black text-[#1B2559]">File Details</h3>
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Manage Asset & SEO</p>
                                </div>
                                <button @click="selectedFile = null" class="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center hover:bg-slate-100 text-slate-400 transition-colors">
                                    <X :size="18" />
                                </button>
                            </div>
                            ...
                            <div class="flex-1 overflow-y-auto p-6 space-y-6">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">File Size</p>
                                        <p class="text-xs font-bold text-[#1B2559]">{{ formatBytes(selectedFile.size) }}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Type</p>
                                        <p class="text-xs font-bold text-[#1B2559] uppercase">{{ selectedFile.name.split('.').pop() || 'IMG' }}</p>
                                    </div>
                                </div>

                                <div>
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">File Name</label>
                                    <div class="p-3 bg-white border border-slate-200 rounded-xl text-xs font-medium text-slate-600 break-all">
                                        {{ selectedFile.name }}
                                    </div>
                                </div>

                                <div>
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Alt Text (SEO)</label>
                                    <textarea v-model="selectedFile.alt" rows="5" 
                                        ...
                                        placeholder="Describe this image for search engines..."></textarea>
                            ...
                            <div class="p-6 border-t border-slate-100 bg-slate-50 flex gap-3">
                                <button @click="handleDeleteMedia(selectedFile.id); selectedFile = null" 
                                ...
                                <button @click="saveAltText(selectedFile)" :disabled="savingAlt"
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
                enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in"
                leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="showMoveModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="showMoveModal = false"></div>
                    <div class="relative bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
                        <h2 class="text-xl font-black text-[#1B2559] mb-2">Move File</h2>
                        <p class="text-sm text-slate-500 mb-6">Moving: <span class="font-bold text-[#1B2559]">{{
                            moveTarget?.name }}</span></p>
                        <div>
                            <label
                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Destination
                                Folder</label>
                        ...
                        <div class="flex gap-3 mt-8">
                            <BaseButton variant="secondary" class="flex-1" @click="showMoveModal = false">Cancel
                            </BaseButton>
                            <BaseButton variant="primary" class="flex-1" @click="moveFile">Move Here</BaseButton>
                        </div>
                    </div>
                </div>
            </Transition>
...
import {
    Upload, Plus, Copy, Trash2, FolderPlus, Folder, FolderInput,
    ChevronRight, Search, Grid3x3, List, Database, Image as ImageIcon, X, Info, ExternalLink
...
import { sanityClient, sanityWriteClient } from '../services/sanity'
...
const folders = ref<MediaFolder[]>([
    ...
    { id: 'blog-covers', name: 'Covers', parentId: 'blog', createdAt: '2026-01-01' },
    { id: 'blog-content', name: 'Content Images', parentId: 'blog', createdAt: '2026-01-01' },
    { id: 'portfolio', name: 'Portfolio', parentId: null, createdAt: '2026-01-01' },
    { id: 'portfolio-thumbnails', name: 'Thumbnails', parentId: 'portfolio', createdAt: '2026-01-01' },
    { id: 'portfolio-galleries', name: 'Galleries', parentId: 'portfolio', createdAt: '2026-01-01' },
    { id: 'products', name: 'Products', parentId: null, createdAt: '2026-01-01' },
    { id: 'products-ebooks', name: 'E-Books', parentId: 'products', createdAt: '2026-01-01' },
    { id: 'products-courses', name: 'E-Courses', parentId: 'products', createdAt: '2026-01-01' },
    { id: 'branding', name: 'Branding', parentId: null, createdAt: '2026-01-01' },
    { id: 'branding-logos', name: 'Logos', parentId: 'branding', createdAt: '2026-01-01' },
    { id: 'branding-icons', name: 'Icons', parentId: 'branding', createdAt: '2026-01-01' },
...
const totalSize = computed(() => mediaItems.value.reduce((sum, m) => sum + m.size, 0))
...
const getFolderItemCount = (folderId: string): number => {
    ...
    return mediaItems.value.filter(m => folderIds.includes(m.folderId || '')).length
}
...
const handleFileUpload = async (e: Event) => {
    ...
    try {
        for (const file of Array.from(files)) {
            ...
            const asset = await sanityWriteClient.assets.upload('image', optimizedFile, {
...
const handleDeleteFolder = (folderId: string) => {
    ...
    if (selectedFolderId.value && descendantIds.includes(selectedFolderId.value)) {
...
const handleDeleteMedia = async (mediaId: string) => {
    ...
    try {
        await sanityWriteClient.delete(mediaId)
    } catch (e) {
...
const moveFile = async () => {
    ...
    try {
        ...
        await sanityWriteClient.patch(targetId)
            .set(update)
            .commit()
        ...
        showToast('Failed to save move action: ' + (e instanceof Error ? e.message : 'Unknown'), 'error')
        // Revert local state
...
const saveAltText = async (item: MediaItem) => {
    ...
    try {
        await sanityWriteClient.patch(item.id)
            .set({ alt: item.alt || '' })
            .commit()
