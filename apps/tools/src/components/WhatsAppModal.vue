<template>
  <BottomSheet :modelValue="isOpen" :title="title || 'WhatsApp Consultation'" :icon="MessageCircle"
    ...
    <div class="p-6">
      ...
      <div class="mb-8">
        <div class="flex flex-col items-start max-w-[90%]">
          <div
            ...
            <p>{{ displayGreeting }}</p>
            <span class="block text-[0.7rem] text-text-tertiary font-bold mt-2 text-right uppercase tracking-wider">{{
              currentTime }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-5">
        <div class="space-y-2">
          <label class="block text-[0.7rem] font-black uppercase tracking-widest text-text-tertiary px-1">Nama
            Lengkap</label>
          <input v-model="form.name" type="text" placeholder="Masukkan nama Anda..."
        ...
        <div class="space-y-2">
          <label class="block text-[0.7rem] font-black uppercase tracking-widest text-text-tertiary px-1">Pesan
            Anda</label>
          <textarea v-model="form.message" placeholder="Tulis detail project atau pertanyaan..." rows="4"
            class="w-full bg-bg-primary border border-border-color rounded-2xl px-5 py-4 text-text-primary text-[0.95rem] outline-none transition-all focus:border-green-500/50 focus:ring-4 focus:ring-green-500/5 resize-none placeholder:text-text-tertiary"></textarea>
        </div>
      </div>
    ...
    <!-- Footer Action -->
    <template #footer>
      <button @click="handleSend" :disabled="!form.name.trim() || !form.message.trim() || loading"
...
const displayGreeting = computed(() => {
  return props.bubbleMessage || 'Halo! Saya siap membantu diskusi project Anda. Silakan isi pesan di bawah dan klik kirim untuk terhubung ke WhatsApp saya.';
});

// Set initial message when modal opens
watch(() => props.isOpen, (val) => {
  if (val) {
    form.value.message = props.initialMessage || siteConfig.whatsapp.defaultMessage;
  }
});
